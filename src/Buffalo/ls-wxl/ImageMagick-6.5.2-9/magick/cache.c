...
static IndexPacket *GetAuthenticIndexesFromCache(const Image *image)
{
  ...
  if (image->debug != MagickFalse)
    (void) LogMagickEvent(TraceEvent,GetMagickModule(),"%s",image->filename);
  cache_info=(CacheInfo *) image->cache;
  indexes=GetPixelCacheNexusIndexes(image->cache,
    cache_info->nexus_info[GetPixelCacheThreadId()]);
  return(indexes);
}

/*
...
static PixelPacket *GetAuthenticPixelsFromCache(const Image *image)
{
  ...
  if (image->debug != MagickFalse)
    (void) LogMagickEvent(TraceEvent,GetMagickModule(),"%s",image->filename);
  cache_info=(CacheInfo *) image->cache;
  pixels=GetPixelCacheNexusPixels(image->cache,
    cache_info->nexus_info[GetPixelCacheThreadId()]);
  return(pixels);
}

  ...
  ExceptionInfo *exception)
{
  ...
  pixels=GetAuthenticPixelCacheNexus(image,x,y,columns,rows,
    cache_info->nexus_info[GetPixelCacheThreadId()],exception);
  return(pixels);
}

/*
...
MagickExport MagickSizeType GetImageExtent(const Image *image)
{
  ...
  cache_info=(CacheInfo *) image->cache;
  assert(cache_info->signature == MagickSignature);
  extent=GetPixelCacheNexusExtent(image->cache,
    cache_info->nexus_info[GetPixelCacheThreadId()]);
  return(extent);
}

...
static const IndexPacket *GetVirtualIndexesFromCache(const Image *image)
{
  ...
  if (image->debug != MagickFalse)
    (void) LogMagickEvent(TraceEvent,GetMagickModule(),"%s",image->filename);
  cache_info=(CacheInfo *) image->cache;
  indexes=GetVirtualIndexesFromNexus(image->cache,
    cache_info->nexus_info[GetPixelCacheThreadId()]);
  return(indexes);
}

...
{
  return((long) (columns*GetPseudoRandomValue()));
}

static inline long RandomY(const unsigned long rows)
{
  return((long) (rows*GetPseudoRandomValue()));
}
...
static const PixelPacket *GetVirtualPixelsCache(const Image *image)
{
  ...
  if (image->debug != MagickFalse)
    (void) LogMagickEvent(TraceEvent,GetMagickModule(),"%s",image->filename);
  cache_info=(CacheInfo *) image->cache;
  pixels=GetVirtualPixelsNexus(image->cache,
    cache_info->nexus_info[GetPixelCacheThreadId()]);
  return(pixels);
}

  ...
  ExceptionInfo *exception)
{
  ...
  if (cache_info == (Cache) NULL)
    return((PixelPacket *) NULL);
  pixels=QueueAuthenticNexus(image,x,y,columns,rows,
    cache_info->nexus_info[GetPixelCacheThreadId()],exception);
  return(pixels);
}

  ...
  ExceptionInfo *exception)
{
  ...
  cache_info=(CacheInfo *) image->cache;
  status=SyncAuthenticPixelCacheNexus(image,
    cache_info->nexus_info[GetPixelCacheThreadId()],exception);
  return(status);
}

