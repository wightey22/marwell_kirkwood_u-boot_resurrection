  ...
  ExceptionInfo *exception)
{
  ...
  switch (function)
  {
    ...
    case ArcsinFunction:
    {
      ...
      if ( result <= -1.0 )
        result = MagickEpsilon - 1.0;
      if ( result >= 1.0 )
        result = 1.0 - MagickEpsilon;
      result=(MagickRealType) (QuantumRange*(range/MagickPI*asin((double)
                   result) + bias ) );
      break;
    }
    case ArctanFunction:
    {
      ...
      range  = ( number_parameters >= 3 ) ? parameters[2] : 1.0;
      bias   = ( number_parameters >= 4 ) ? parameters[3] : 0.5;
      result = 2.0*slope*(QuantumScale*pixel - center);
  ...
  */
  ...
  for (i=depth-1; (i >= 0) && (j < (long) depth); i--)
  {
    ...
    if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
      ...
      {
        status=image->progress_monitor(SteganoImageTag,depth-i,depth,
          image->client_data);
        if (status == MagickFalse)
          break;
      }
