...
static int audit_connect(vfs_handle_struct *handle, const char *svc, const char *user)
{
	...
	syslog(audit_syslog_priority(handle), "connect to service %s by user %s\n", 
	       svc, user);
	#else
        openlog("file_smb", LOG_PID, audit_syslog_facility(handle));
	#endif
...
static int audit_mkdir(vfs_handle_struct *handle, const char *path, mode_t mode)
{
	...
	#else
        syslog(audit_syslog_priority(handle), "[User:%s(%s)] Make Directory %s/%s %s%s\n",
               handle->conn->session_info->unix_name, handle->conn->sconn->client_id.addr,
...
static int audit_rmdir(vfs_handle_struct *handle, const char *path)
{
	...
	syslog(audit_syslog_priority(handle), "rmdir %s %s%s\n", 
	       ...
	       (result < 0) ? strerror(errno) : "");
	#else
        syslog(audit_syslog_priority(handle), "[User:%s(%s)] Remove Directory %s/%s %s%s\n", 
               handle->conn->session_info->unix_name, handle->conn->sconn->client_id.addr,
               handle->conn->connectpath, path,
               (result < 0) ? "failed: " : "",
               (result < 0) ? strerror(errno) : "");
		      ...
		      int flags, mode_t mode)
{
	...
	syslog(audit_syslog_priority(handle), "open %s (fd %d) %s%s%s\n", 
	       ...
	       (result < 0) ? strerror(errno) : "");
	#else
        syslog(audit_syslog_priority(handle), "[User:%s(%s)] File Open(%s) %s/%s %s%s\n", 
               handle->conn->session_info->unix_name, handle->conn->sconn->client_id.addr,
               ((flags & O_WRONLY) || (flags & O_RDWR)) ? "Read/Write" : "Read",
			...
			const struct smb_filename *smb_fname_dst)
{
	...
	syslog(audit_syslog_priority(handle), "rename %s -> %s %s%s\n",
	       ...
	       (result < 0) ? strerror(errno) : "");
	#else
        syslog(audit_syslog_priority(handle), "[User:%s(%s)] Rename File %s/%s -> %s/%s %s%s\n",
               handle->conn->session_info->unix_name, handle->conn->sconn->client_id.addr,
               handle->conn->connectpath, smb_fname_src->base_name,
               handle->conn->connectpath, smb_fname_dst->base_name,
               (result < 0) ? "failed: " : "",
               (result < 0) ? strerror(errno) : "");
	#endif
			...
			const struct smb_filename *smb_fname)
{
	...
	syslog(audit_syslog_priority(handle), "unlink %s %s%s\n",
	       ...
	       (result < 0) ? strerror(errno) : "");
	#else
        syslog(audit_syslog_priority(handle), "[User:%s(%s)] Remove File %s/%s %s%s\n",
               handle->conn->session_info->unix_name, handle->conn->sconn->client_id.addr,
               handle->conn->connectpath, smb_fname->base_name,
               (result < 0) ? "failed: " : "",
               (result < 0) ? strerror(errno) : "");
