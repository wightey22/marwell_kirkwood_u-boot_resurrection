...
static uint32_t ldapsam_algorithmic_capabilities(struct pdb_methods *methods)
{
	return PDB_CAP_ALGORITHMIC_RIDS;
}
...
NTSTATUS pdb_init_ldapsam(struct pdb_methods **pdb_method, const char *location)
{
	...
	if (alg_rid_base_string) {
		...
		(*pdb_method)->capabilities = ldapsam_algorithmic_capabilities;
	} else {
		next_rid_string = smbldap_talloc_single_attribute(
			ldap_state->smbldap_state->ldap_struct,
                        entry,
			get_attr_key2string( dominfo_attr_list,
					     LDAP_ATTR_NEXT_RID ),
