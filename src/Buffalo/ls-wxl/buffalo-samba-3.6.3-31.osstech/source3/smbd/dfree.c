...
****************************************************************************/
bool is_enough_dfree(connection_struct *conn,
		     const char *path,
		     uint64_t size)
{
	...
	size /= 1024; /* size is now number of 1k blocks needed */
	space_avail = get_dfree_info(conn, path, False, &bsize, &dfree, &dsize);
	DEBUG(10, ("path [%s], size [%lld], space_savil [%lld]\n", path, size, space_avail));
