				     ...
				     int *pinfo)
{
	...
	if (create_options & FILE_DIRECTORY_FILE) {
		 ...
		 */
                ...
                /* 
		 * Check enough free space on the disk before opening 
		 * a new file 
		 * Windows 2003 does not send SMB_SET_FILE_END_OF_FILE_INFO
		 * message.
		 */
		
                if (lp_strict_allocate(SNUM(conn))){
                        uint64_t need_size = 0;
                        char * fspath;
                        DEBUG(5, ("create_disposition:%d, allocation_size:%lld\n", create_disposition, allocation_size));

                        if (allocation_size){
                                if (VALID_STAT(smb_fname->st)){
                                        /* file exist */
                                        if (allocation_size > smb_fname->st.st_ex_size){
                                                need_size = allocation_size - (uint64_t)smb_fname->st.st_ex_size;
                                        }
                                        fspath = smb_fname->base_name;
                                } else {
                                        if (!parent_dirname(talloc_tos(), smb_fname->base_name, &fspath, NULL)) {
                                                return NT_STATUS_NO_MEMORY;
                                        }
                                        need_size = allocation_size;
                                }
                                if (need_size > 0){
                                        need_size = smb_roundup(conn, need_size);
                                        if (!is_enough_dfree(conn, fspath, need_size)){
                                                DEBUG(5, ("%s does not have enough space [%lld].\n", fspath, need_size));
