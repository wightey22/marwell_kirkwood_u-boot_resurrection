...
void start_smbd(void)
{
	...
	argv[0] = binfile;
	if (!(args = getenv("SWAT_SMBD_OPTIONS"))) {
		argv[1] = "-D";
		argv[2] = NULL;
	} else {
		argv[1] = strtok(args, ARGV_DELIM);
		for (argc = 2; argc < ARGC_MAX; argc++) {
			if (!(argv[argc] = strtok(NULL, ARGV_DELIM))) {
				break;
			}
		}
		argv[argc] = NULL;
...
void start_nmbd(void)
{
	...
	argv[0] = binfile;
	if (!(args = getenv("SWAT_NMBD_OPTIONS"))) {
		argv[1] = "-D";
		argv[2] = NULL;
	} else {
		argv[1] = strtok(args, ARGV_DELIM);
...
void start_winbindd(void)
{
	...
	binfile = getenv("SWAT_WINBINDD_PATH");
	if (!binfile) {
	...
	argv[0] = binfile;
	if (!(args = getenv("SWAT_WINBINDD_OPTIONS"))) {
		argv[1] = "-D";
		argv[2] = NULL;
	} else {
		argv[1] = strtok(args, ARGV_DELIM);
