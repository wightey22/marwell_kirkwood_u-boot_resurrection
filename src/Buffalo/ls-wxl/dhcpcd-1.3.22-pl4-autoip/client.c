...
#endif
#ifdef BUFFALO_PLATFORM
extern int		DontDownIface;  // from dhcpcd.c
extern int		alwaysFork;
extern int		DhcpRetryInterval_sec;
#endif
...
#endif
/*****************************************************************************/
int dhcpSendAndRecv2(xid,msg,buildUdpIpMsg)
unsigned xid,msg;
void (*buildUdpIpMsg)(unsigned);
{
  struct sockaddr addr;
  ...
  int send_count, sync;
	for(loop = 0; loop <2 ; loop++)
    {
	  ...
	  gettimeofday(&begin, NULL);
      	  i=random();
      	  send_count --;
      	  sync = peekfd(dhcpSocket,j);
      	  //syslog(LOG_INFO,"sendto:  count %d   sync %d",send_count, sync);
      	  printf("sendto:  count %d   sync %d\n",send_count, sync);
    	}
      while ( send_count > 0 && sync );
      if(sync) return 1;

if (fcntl(dhcpSocket, F_SETFL, O_NONBLOCK) == -1)
{
...
FUNCTION;
	  ...
	  gettimeofday(&current, NULL);
	  timeval_subtract(&diff, &current, &begin);
	  timeout = j - diff.tv_sec*1000000 - diff.tv_usec;
	  if ( UdpIpMsgRecv.ethhdr.ether_type != htons(ETHERTYPE_IP) )
	    continue;
	  tmp_ip = UdpIpMsgRecv.udpipmsg;
	  for (i=0;i<sizeof(struct ip)-2;i+=2)
...
DhcpIface.client_id[6],DhcpIface.client_id[7],DhcpIface.client_id[8]);
  ...
  ret = dhcpSendAndRecv2(random(),DHCP_OFFER,&buildDhcpDiscover);
  if ( ret == 1 )
    {
      printf("dhcpInit call AutoipBound	\n");
      //dhcpStop();
      //if (conflictIP_flag) dhcpStop();
//      if(setAutoIP() || alwaysFork) return &AutoipBound;
     if(setAutoIP(0)) return &AutoipBound;
     if (alwaysFork){
		execute_on_change("ipfail_fork");
		//printf("ipfail_fork\n");
		return &AutoipBound;
	 }
      dhcpStop();
...
FUNCTION;
      ...
      dhcpSendAndRecv2(DhcpMsgRecv->xid,DHCP_OFFER,&buildDhcpDiscover);
    }
...
/* send the message and read and parse replies into DhcpOptions */
  ...
  if ( dhcpSendAndRecv2(xid,DHCP_ACK,buildDhcpMsg) ) return &dhcpInit;
...
{
FUNCTION;
  //if ( sigsetjmp(env,0xffff) ) return &dhcpReboot;
  //conflictIP();
  //return &dhcpReboot;
  ...
  //  conflictIP_flag = 0;
  //  if ( sigsetjmp(env,0xffff) ) return &dhcpReboot;
  //  conflictIP_flag = conflictIP();
  //  return &dhcpReboot;
  //}
  //else
  //{
  //  if ( sigsetjmp(env,0xffff) ) return &dhcpInit;
  //  conflictIP_flag = conflictIP();
  //  return &dhcpInit;
  //}
  ...
  printf("waiting %dsec...\n",DhcpRetryInterval_sec);
#if 1
  if ( conflictIP(DhcpRetryInterval_sec) )
  {
    dhcpStop();
    return &dhcpReboot;
  }
#else
  sleep(DhcpRetryInterval_sec);			// next discover retry 
#endif
  return &dhcpInit;
...
FUNCTION;
  ...
  if ( dhcpSendAndRecv2(random(),DHCP_ACK,&buildDhcpRenew) ) return &dhcpRebind;
  ReqSentTime=time(NULL);
  if ( DebugFlag ) syslog(LOG_DEBUG,
    "DHCP_ACK received from %s (%u.%u.%u.%u)\n",DhcpMsgRecv->sname,
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[0],
...
FUNCTION;
  ...
  if ( DebugFlag )
    syslog(LOG_DEBUG,"broadcasting DHCP_REQUEST for %u.%u.%u.%u\n",
	   ...
	   ((unsigned char *)&DhcpIface.ciaddr)[2],
	   ((unsigned char *)&DhcpIface.ciaddr)[3]);
  if ( dhcpSendAndRecv2(random(),DHCP_ACK,&buildDhcpRebind) ) return &dhcpStop;
  ReqSentTime=time(NULL);
  if ( DebugFlag ) syslog(LOG_DEBUG,
    "DHCP_ACK received from %s (%u.%u.%u.%u)\n",DhcpMsgRecv->sname,
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[0],
...
#endif
#ifdef BUFFALO_PLATFORM
syslog(LOG_DEBUG,"** DontDownIface=%d\n",DontDownIface);
  if (DontDownIface){
    ifr.ifr_flags = saved_if_flags;
  }else{
...
FUNCTION;
  ...
  if ( DebugFlag )
    syslog(LOG_DEBUG,"broadcasting DHCP_INFORM for %u.%u.%u.%u\n",
	   ...
	   ((unsigned char *)&DhcpIface.ciaddr)[2],
	   ((unsigned char *)&DhcpIface.ciaddr)[3]);
  if ( dhcpSendAndRecv2(random(),DHCP_ACK,buildDhcpInform) ) return 0;
  if ( DebugFlag ) syslog(LOG_DEBUG,
    "DHCP_ACK received from %s (%u.%u.%u.%u)\n",DhcpMsgRecv->sname,
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[0],
    ((unsigned char *)DhcpOptions.val[dhcpServerIdentifier])[1],
