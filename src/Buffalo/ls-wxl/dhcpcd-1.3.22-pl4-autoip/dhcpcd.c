...
int		DontDownIface=0;	// buffalo
int		alwaysFork=0;	// buffalo
int		DhcpRetryInterval_sec=5*60;
#define AUTOIP_VERSION	"autoip 2"
#endif
...
#ifdef BUFFALO_PLATFORM
  fprintf(stderr,"\
DHCP Client Daemon v."VERSION" "AUTOIP_VERSION" (BUFFALO modified " __DATE__ ")\n\
Copyright (C) 1996 - 1997 Yoichi Hariguchi <yoichi@fore.com>\n\
Copyright (C) January, 1998 Sergei Viznyuk <sv@phystech.com>\n\
...
void checkIfAlreadyRunning()
{
  int o;
  char pidfile[64];
  snprintf(pidfile,sizeof(pidfile),PID_FILE_PATH,PID_FILE_DIR,IfNameExt);
...
prgs: switch ( argc[i][s] )
	{
	  ...
	  case 'z':
	    ...
	    i++;
	    if ( argc[i] )
	      DhcpRetryInterval_sec=atol(argc[i++]);
	    else
	      goto usage;
	    s=1;
	    if ( DhcpRetryInterval_sec > 0 ) break;
	    goto usage;
...
//  alarm(TimeOut);
    ...
    {
	printf("dhcpcd.c 376: dhcpcd error\n");
	 exit(1);
	}
  while ( (currState != &dhcpBound) && (currState !=AutoipBound));
//  while ( (currState != &dhcpBound));
  ...
  setsid();
  if ( DebugFlag==0 && (i=open("/dev/null",O_RDWR,0)) >= 0 )
    {
