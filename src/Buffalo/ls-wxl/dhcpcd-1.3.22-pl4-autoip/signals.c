      	...
      	else
       	{
          	...
          	else
	    	{
	      		...
	      		else
				{
		  			...
		  			{
			  			if ( currState == &dhcpInit )
						{

	            	syslog(LOG_ERR,"timed out waiting for a valid DHCP server response dhcpInit\n");
							...
							if(!autoip_doing){
								...
								deletePidFile();
								if(setAutoIP()){
									system("/sbin/autoipd eth0 HD-HGLAND00 &");
									exit(0);
								}
							}
