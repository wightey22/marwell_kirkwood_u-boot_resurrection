...
#undef BUFFALO_DEBUG
  ...
  #define BDP(fmt,args...)	printf(fmt,##args)
#else
  #define BDP(fmt,args...)
#endif

#define BUFFALO_COMPRESS
...
static int compressLogFile(char * name, logInfo * log, struct stat *sb) {
    ...
    fullCommand = alloca(sizeof(*fullCommand) * 
			 (log->compress_options_count + 3));
    fullCommand[0] = log->compress_prog;
    for (i = 0; i < log->compress_options_count; i++)
    {
	fullCommand[i + 1] = log->compress_options_list[i];
    }
    fullCommand[log->compress_options_count + 1] = name;	//BUFFALO transfer filename by fullCommnad
    fullCommand[log->compress_options_count + 2] = NULL;
    ...
    if (!fork()) {
/*	BUFFALO transfer filename by fullCommnad
	dup2(inFile, 0);
	close(inFile);
	dup2(outFile, 1);
	close(outFile);
*/
    ...
    if (!WIFEXITED(status) || WEXITSTATUS(status)) {
        BDP("WEXITSTATUS(status)=%d\n", WEXITSTATUS(status));
	message(MESS_ERROR, "failed to compress log %s\n", name);
	return 1;
    }
  
    int outFile;
    if ((outFile = open(compressedName, O_RDWR , sb->st_mode)) < 0) {
...
static int compressLogFile(char * name, logInfo * log, struct stat *sb) {
    ...
    fullCommand = alloca(sizeof(*fullCommand) * 
			 (log->compress_options_count + 3));
    fullCommand[0] = log->compress_prog;
    BDP("log->compress_prog=%s\n", log->compress_prog);
    for (i = 0; i < log->compress_options_count; i++)
    {
	fullCommand[i + 1] = log->compress_options_list[i];
        BDP("log->compress_options_list[%d]=%s\n", i, log->compress_options_list[i]);
    }
    fullCommand[log->compress_options_count + 1] = name;	//BUFFALO transfer filename by fullCommnad
    BDP("name=%s\n", name);
    fullCommand[log->compress_options_count + 2] = NULL;
