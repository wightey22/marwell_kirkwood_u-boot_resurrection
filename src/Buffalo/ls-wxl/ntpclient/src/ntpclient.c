...
#include <syslog.h>
#define LOG( str, args...)  SYSLOG(LOG_MAKEPRI(LOG_USER,LOG_NOTICE), str, ## args)
#define LOG_E( str, args...)  SYSLOG(LOG_MAKEPRI(LOG_USER,LOG_ERR), str, ## args)

#define OPEN_LOG(name) openlog(name, 0, LOG_USER)
...
#undef Data
	...
	if (set_clock) {   /* you'd better be root, or ntpclient will crash! */
		...
		{// add LOG NTTDATA-ITEC
			char timestamp[32];
			time_t now;
			time(&now);
			if(getenv("TZOFFMIN")){
				now += (atoi(getenv("TZOFFMIN")) * 60);//set TZOFFMIN in bufapli/rc/src/rc_tz.c
			}
			ctime_r(&now, timestamp);
			LOG("SUCCESS: set time :  %s", timestamp);
			printf("SUCCESS\n");
		}
		if (debug) {
...
//Append by terasawa@nttdata-itec 2006/02/08
void select_sleep(int interval)
{
	struct timeval select_sleep;
	select_sleep.tv_sec  = interval;
	select_sleep.tv_usec = 0;
	select(0, NULL, NULL, NULL, &select_sleep);
}
//Append by terasawa@nttdata-itec 2006/02/08

//modified by terasawa@nttdata-itec 2006/02/08
void stuff_net_addr(struct in_addr *p, char *hostname)
{
	struct hostent *ntpserver;
	//–¼‘O‰ðŒˆ‚ÉŽ¸”s‚µ‚½ê‡10•ªŠÔŠu‚ÅÄ“x–â‚¢‡‚í‚¹‚·‚é
	for(;;){
		ntpserver=gethostbyname(hostname);
		if (ntpserver == NULL) {
			herror(hostname);
			//LOG_E("Error looking up hostname %s\n", hostname);
			select_sleep(600);
			continue;
		}
...
void primary_loop(int usd, int num_probes, int interval, int goodness)
{
	...
	for (;;) {
		...
		if ((i!=1)||(!FD_ISSET(usd,&fds))) {
		  //if can't found server,ask every 10 seconds
		  //yukihisa@NTTDATA-ITEC
		  if (i == 0) {
		    send_packet(usd);
		    ++probes_sent;
		    to.tv_sec = 10;
...
int main(int argc, char *argv[]) {
	...
	LOG("probe_count=%d  hostname=%s  cycle_time=%d\n"
		,probe_count
		,hostname
		,cycle_time);
