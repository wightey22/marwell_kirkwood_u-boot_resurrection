...
int doget(const char *name, const char *kbuf, int ksiz, int start, int max, int ox, int nb);
int dolist(const char *name, int kb, int vb, int ox);
...
void usage(void){
  ...
  fprintf(stderr, "  %s get [-kx|-ki] [-start num] [-max num] [-ox] [-n] name key\n",
          progname);
  fprintf(stderr, "  %s list [-k|-v] [-ox] name\n", progname);
...
int runget(int argc, char **argv){
  ...
  if(kbuf){
    rv = doget(name, kbuf, ksiz, start, max, ox, nb);
  } else {
    fprintf(stderr, "%s: out of memory\n", progname);
    rv = 1;
  }
