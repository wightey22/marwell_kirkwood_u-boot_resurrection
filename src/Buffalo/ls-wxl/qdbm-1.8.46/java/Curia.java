...
public class Curia implements ADBM {
   ...
   */
  public void put(byte[] key, byte[] val, int dmode) throws CuriaException {
    if(index < 0) throw new CuriaException();
    synchronized(ADBM.class){
   ...
   */
  public void put(byte[] key, byte[] val) throws CuriaException {
    put(key, val, DOVER);
  }
  /**
   * Delete a record.
   ...
   */
  public void out(byte[] key) throws CuriaException {
    if(index < 0) throw new CuriaException();
    synchronized(ADBM.class){
      if(crout(index, key, key.length) == 0) throw new CuriaException(crecode());
    }
  }
  /**
   * Retrieve a record.
   ...
   * @param key a byte array of a key.
   * @return the size of the value of the corresponding record.
   * @throws CuriaException if an error occurs or no record corresponds.
   ...
   * @throws CuriaException if an error occurs or replace is cancelled.
   */
  public void putlob(byte[] key, byte[] val, int dmode) throws CuriaException {
    if(index < 0) throw new CuriaException();
    synchronized(ADBM.class){
   ...
   * @see #put(byte[], byte[], int)
   */
  public void putlob(byte[] key, byte[] val) throws CuriaException {
    putlob(key, val, DOVER);
  }
  /**
   ...
   */
  public void outlob(byte[] key) throws CuriaException {
    if(index < 0) throw new CuriaException();
    synchronized(ADBM.class){
      if(croutlob(index, key, key.length) == 0) throw new CuriaException(crecode());
    }
  }
  /**
   ...
   * @return a byte array of the value of the corresponding large object.
