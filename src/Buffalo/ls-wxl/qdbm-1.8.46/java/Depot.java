...
public class Depot implements ADBM {
   ...
   */
  public void put(byte[] key, byte[] val, int dmode) throws DepotException {
    if(index < 0) throw new DepotException();
    synchronized(ADBM.class){
   ...
   */
  public void put(byte[] key, byte[] val) throws DepotException {
    put(key, val, DOVER);
  }
  /**
   * Delete a record.
   ...
   */
  public void out(byte[] key) throws DepotException {
    if(index < 0) throw new DepotException();
    synchronized(ADBM.class){
      if(dpout(index, key, key.length) == 0) throw new DepotException(dpecode());
    }
  }
  /**
   * Retrieve a record.
