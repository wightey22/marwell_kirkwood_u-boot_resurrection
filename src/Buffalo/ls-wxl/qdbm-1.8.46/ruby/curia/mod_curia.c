...
static VALUE rbcrput(VALUE vself, VALUE vindex, VALUE vkey, VALUE vval, VALUE vdmode){
  ...
  curia = crtable[index];
  if(!crput(curia, kbuf, ksiz, vbuf, vsiz, dmode)) myerror(dpecode);
  return Qtrue;
}
...
static VALUE rbcrout(VALUE vself, VALUE vindex, VALUE vkey){
  ...
  ksiz = RSTRING(vkey)->len;
  curia = crtable[index];
  if(!crout(curia, kbuf, ksiz)) myerror(dpecode);
  return Qtrue;
}
...
static VALUE rbcrget(VALUE vself, VALUE vindex, VALUE vkey, VALUE vstart, VALUE vmax){
  ...
  max = FIX2INT(vmax);
  curia = crtable[index];
  if(!(vbuf = crget(curia, kbuf, ksiz, start, max, &vsiz))) myerror(dpecode);
...
static VALUE rbcrvsiz(VALUE vself, VALUE vindex, VALUE vkey){
  ...
  ksiz = RSTRING(vkey)->len;
  curia = crtable[index];
  if((vsiz = crvsiz(curia, kbuf, ksiz)) == -1) myerror(dpecode);
  return INT2FIX(vsiz);
}
...
static VALUE rbcriternext(VALUE vself, VALUE vindex){
  ...
  if((index = FIX2INT(vindex)) == -1) myerror(DP_EMISC);
  curia = crtable[index];
  if(!(kbuf = criternext(curia, &ksiz))) myerror(dpecode);
