...
static VALUE rbdpput(VALUE vself, VALUE vindex, VALUE vkey, VALUE vval, VALUE vdmode){
  ...
  depot = dptable[index];
  if(!dpput(depot, kbuf, ksiz, vbuf, vsiz, dmode)) myerror(dpecode);
  return Qtrue;
}
...
static VALUE rbdpout(VALUE vself, VALUE vindex, VALUE vkey){
  ...
  ksiz = RSTRING(vkey)->len;
  depot = dptable[index];
  if(!dpout(depot, kbuf, ksiz)) myerror(dpecode);
  return Qtrue;
}
...
static VALUE rbdpget(VALUE vself, VALUE vindex, VALUE vkey, VALUE vstart, VALUE vmax){
  ...
  max = FIX2INT(vmax);
  depot = dptable[index];
  if(!(vbuf = dpget(depot, kbuf, ksiz, start, max, &vsiz))) myerror(dpecode);
...
static VALUE rbdpvsiz(VALUE vself, VALUE vindex, VALUE vkey){
  ...
  ksiz = RSTRING(vkey)->len;
  depot = dptable[index];
  if((vsiz = dpvsiz(depot, kbuf, ksiz)) == -1) myerror(dpecode);
  return INT2FIX(vsiz);
}
...
static VALUE rbdpiternext(VALUE vself, VALUE vindex){
  ...
  if((index = FIX2INT(vindex)) == -1) myerror(DP_EMISC);
  depot = dptable[index];
  if(!(kbuf = dpiternext(depot, &ksiz))) myerror(dpecode);
