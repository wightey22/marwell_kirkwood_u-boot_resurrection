...
VILLA *vlopen(const char *name, int omode, VLCFUNC cmp){
  ...
  if(omode & VL_OWRITER){
    ...
    if(_qdbm_deflate && cmode == VL_OZCOMP){
      ...
      flags |= VL_FLISLZO;
    } else if(_qdbm_lzoencode && cmode == VL_OXCOMP){
      flags |= VL_FLISBZIP;
    }
...
int vlvsiz(VILLA *villa, const char *kbuf, int ksiz){
  ...
  assert(villa && kbuf);
  if(ksiz < 0) ksiz = strlen(kbuf);
  if((pid = vlsearchleaf(villa, kbuf, ksiz, NULL, NULL)) == -1) return -1;
...
int vlcurput(VILLA *villa, const char *vbuf, int vsiz, int cpmode){
  ...
  case VL_CPBEFORE:
    if(villa->curvnum < 1){
      if(!recp->rest){
        tbuf = cbdatumtomalloc(recp->first, &tsiz);
        recp->first = cbdatumopen(vbuf, vsiz);
        recp->rest = cblistopen();
        cblistpushbuf(recp->rest, tbuf, tsiz);
      } else {
        cblistunshift(recp->rest, CB_DATUMPTR(recp->first), CB_DATUMSIZE(recp->first));
        cbdatumsetsize(recp->first, 0);
        cbdatumcat(recp->first, vbuf, vsiz);
      }
    } else {
      cblistinsert(recp->rest, villa->curvnum - 1, vbuf, vsiz);
    }
    villa->rnum++;
    break;
  case VL_CPAFTER:
    if(!recp->rest) recp->rest = cblistopen();
    cblistinsert(recp->rest, villa->curvnum, vbuf, vsiz);
    villa->curvnum++;
    villa->rnum++;
    break;
  default:
