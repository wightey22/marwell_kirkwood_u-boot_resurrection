...
int doget(const char *name, const char *kbuf, int ksiz, VLCFUNC cmp, int lb, int ox, int nb);
int dolist(const char *name, const char *tbuf, int tsiz, const char *bbuf, int bsiz,
...
int main(int argc, char **argv){
  ...
  if(!strcmp(argv[1], "create")){
    ...
    printf("Copyright (c) 2000-2006 Mikio Hirabayashi\n");
    rv = 0;
  } else {
    usage();
  }
...
void usage(void){
  ...
  fprintf(stderr, "  %s get [-l] [-kx|-ki] [-ox] [-n] name key\n", progname);
  fprintf(stderr, "  %s list [-k|-v] [-kx|-ki] [-ox] [-top key] [-bot key] [-gt] [-lt]"
          " [-max num] [-desc] name\n", progname);
...
int runget(int argc, char **argv){
  ...
  if(kbuf){
    rv = doget(name, kbuf, ksiz, ki ? VL_CMPINT : VL_CMPLEX, lb, ox, nb);
  } else {
    fprintf(stderr, "%s: out of memory\n", progname);
    rv = 1;
  }
...
int runlist(int argc, char **argv){
  ...
  int i, kb, vb, kx, ki, ox, gt, lt, max, desc, tsiz, bsiz, rv;
  name = NULL;
  kb = FALSE;
  vb = FALSE;
  kx = FALSE;
  ...
  rv = dolist(name, tbuf, tsiz, bbuf, bsiz, ki ? VL_CMPINT : VL_CMPLEX, ki,
