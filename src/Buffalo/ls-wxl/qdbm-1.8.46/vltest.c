...
int dowrite(const char *name, int rnum, int ii, int cmode,
            int lrecmax, int nidxmax, int lcnum, int ncnum);
...
int runwrite(int argc, char **argv){
  char *name, *rstr;
  int i, rnum, ii, cmode, lrecmax, nidxmax, lcnum, ncnum, rv;
  name = NULL;
  rstr = NULL;
  rnum = 0;
  ii = FALSE;
  ...
  rnum = atoi(rstr);
  if(rnum < 1) usage();
  rv = dowrite(name, rnum, ii, cmode, lrecmax, nidxmax, lcnum, ncnum);
  return rv;
}
...
int runrdup(int argc, char **argv){
  char *name, *rstr, *pstr;
  int i, rnum, pnum, ii, cmode, cc, lrecmax, nidxmax, lcnum, ncnum, rv;
  name = NULL;
  rstr = NULL;
  pstr = NULL;
  rnum = 0;
  ...
  rv = dordup(name, rnum, pnum, ii, cmode, cc, lrecmax, nidxmax, lcnum, ncnum);
  return rv;
}
...
int dowrite(const char *name, int rnum, int ii, int cmode,
  ...
  printfflush("<Writing Test>\n  name=%s  rnum=%d  int=%d  cmode=%d  "
              "lrecmax=%d  nidxmax=%d  lcnum=%d  ncnum=%d\n\n",
              name, rnum, ii, cmode, lrecmax, nidxmax, lcnum, ncnum);
  /* open a database */
