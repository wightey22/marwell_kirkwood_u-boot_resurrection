...
#include "case_N.h" /* case 0: cleanup_step++; */
		...
		if (log_ctrl_for_NAS == 1){
			char cmdbuffa[128 + 5 + BIGPATHBUFLEN];
			if(code){
				snprintf(cmdbuffa, sizeof(cmdbuffa), cmdbuff, "(NG)" );
				syslog(LOG_LOCAL4|LOG_DEBUG, "%s", cmdbuffa);

                snprintf(cmdbuff,sizeof(cmdbuff),"detected err(%d)\n",code);
				syslog(LOG_LOCAL4|LOG_DEBUG, "%s", cmdbuff);
			} else {
				snprintf(cmdbuff,sizeof(cmdbuff),"normal end\n");
				syslog(LOG_LOCAL4|LOG_DEBUG, "%s", cmdbuff);
			}
			closelog();
		}
