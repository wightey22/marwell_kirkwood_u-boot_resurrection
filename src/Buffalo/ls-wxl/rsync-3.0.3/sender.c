...
extern int log_ctrl_for_NAS;
extern char cmdbuff[128 + BIGPATHBUFLEN];
...
void send_files(int f_in, int f_out)
{
	...
	if (log_ctrl_for_NAS == 1){
		openlog(RSYNC_LOG, LOG_PID, lp_syslog_facility(LOG_LOCAL4));
	}
	...
	while (1) {
		...
		if (log_ctrl_for_NAS == 1 && strncmp(".", fname, strlen(fname)) !=0){
			snprintf(cmdbuff,sizeof(cmdbuff),"%s %s%s%s", "%s", path,slash,fname);
		}
		...
		if (log_ctrl_for_NAS == 1){
			char cmdbuffa[128 + BIGPATHBUFLEN];
			snprintf(cmdbuffa, sizeof(cmdbuffa), cmdbuff, "(OK)" );
			syslog(LOG_LOCAL4|LOG_DEBUG, "%s", cmdbuffa);
		}
