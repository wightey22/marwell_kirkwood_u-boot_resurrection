...
sub readDirectoryQuery {
	...
	if ($folder eq "/")
	{
		$audiodir = Path::Class::Dir->new($prefs->get("audiodir"));
		$audiodirpar = $audiodir->parent;
		$folder = $audiodirpar->absolute;
		$isaudiodirparent = 1;
	}
	...
	if (main::ISWINDOWS && $folder eq '/') {
		...
		if ($isaudiodirparent == 1)
		{
			my $rel = $audiodir->relative($audiodirpar);
			@fsitems = ("$rel");
		}
