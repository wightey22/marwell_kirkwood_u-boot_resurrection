...
sub init {
	...
	if($prefs->get('language') eq 'UNDEF_LANG')
	{
		my %langmap = (
			'english' => 'EN',
			'japanese' => 'JA',
		...
		open(FI, "cat /etc/melco/info | grep lang | sed -e 's/lang=//' | ");
		@ret = <FI>;
		close(FI);	
		$weblang = $ret[0];
		$weblang =~ s/[ \n\r\f\t]//g;
		if(exists($langmap{$weblang}))
		{
			$squeezelang=$langmap{$weblang};
		}
		else
		{
			$squeezelang='EN';
		}
		$prefs->set('language', $squeezelang);
	}
