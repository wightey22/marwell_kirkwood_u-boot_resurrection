...
sub handler {
	...
	if($paramRef->{'pref_artfolder'} ne '')
	{
		# artfolder must be sub directroy of audiodir
		my $audiodir = Path::Class::Dir->new($prefs->get('audiodir'));
		my $artfolder = Path::Class::Dir->new($paramRef->{'pref_artfolder'});
		my $abs;

		unless ($audiodir->contains($artfolder))
		{
			$abs = $audiodir->absolute;
			$paramRef->{'pref_artfolder'}="$abs";
		}
	}
