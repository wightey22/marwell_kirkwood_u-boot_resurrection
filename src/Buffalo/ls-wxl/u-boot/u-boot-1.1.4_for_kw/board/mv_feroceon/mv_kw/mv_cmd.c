...
int do_print_phy_ident(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	buffalo_print_phy_ident(0);
	buffalo_print_phy_ident(1);
}

U_BOOT_CMD(
        phy_ident, 1, 0, do_print_phy_ident,
        "phy_ident - print phy ident",
        "phy_ident - print phy ident"
);

int do_bar_led_control(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	MV_U8 i = 0;
	MV_U8 value = 0;
	...
	else
	{
		if(strncmp(argv[1], "test", strlen("test")) == 0)
		{
			if(argc == 2)
				...
				bfTestLedBar((MV_16)simple_strtoul(argv[2], NULL, 10), 100);
			else if(argc == 4)
				bfTestLedBar((MV_16)simple_strtoul(argv[2], NULL, 10), (MV_U16)simple_strtoul(argv[3], NULL, 10));
			else
			{
				printf("Argument is wrong\n");
				return -1;
		...
		else if(strncmp(argv[1], "nc", strlen("nc")) == 0)
		{
			if(argc == 2)
				bfNcTestLedBar(10, 100);
			else if(argc == 3)
				bfNcTestLedBar((MV_16)simple_strtoul(argv[2], NULL, 10), 100);
			else if(argc == 4)
				bfNcTestLedBar((MV_16)simple_strtoul(argv[2], NULL, 10), (MV_U16)simple_strtoul(argv[3], NULL, 10));
			else
			{
				printf("Argument is wrong\n");
				return -1;
...
U_BOOT_CMD(
	barled, 4, 0, do_bar_led_control,
	"barled - control bar led (LS-AVL)\n",
	"barled - control bar led (LS-AVL)"
);

int do_bar_led_bright_control(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
...
U_BOOT_CMD(
	ledbc, 1, 0, do_bar_led_bright_control,
	"ledbc - change the led's brightness control. (LS-AVL)\n",
	"ledbc - change the led's brightness control. (LS-AVL)"
);

int do_sw_reg_control(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	ulong i = 0;
	int nbytes = 0;
	extern char console_buffer[];
	unsigned short data = 0, port_num = 0, reg_addr = 0, ope = 0, page_addr = 0;
	...
	do
	{
		...
		{
			if(strncmp(console_buffer, "quit", strlen("quit")) == 0)
				break;
			...
			data = (unsigned short)((i & (0xFFFF << 0)) >> 0);
			port_num = (unsigned short)((i & (0x1F << 16)) >> 16);
			reg_addr = (unsigned short)((i & (0x1F << 21)) >> 21);
			ope = (unsigned short)((i & (0x1 << 26)) >> 26);

			buffaloSwitchPhyRegRead(port_num, 0, 22, &page_addr);
			//printf("port_num = 0x%08x\nreg_addr = 0x%08x\ndata     = 0x%08x\nope      = 0x%08x\n", port_num, reg_addr, data, ope);

			switch(ope)
			{
			case 0:
				// write
				buffaloSwitchPhyRegWrite(port_num, page_addr, reg_addr, data);
			case 1:
				// read
				buffaloSwitchPhyRegRead(port_num, page_addr, reg_addr, &data);
				printf("0x%04x\n", data);
				break;
			default:
				printf("Illigal operation was requested(ope = 0x%08x)\n", ope);
				break;
			}
		}
	} while(1);
