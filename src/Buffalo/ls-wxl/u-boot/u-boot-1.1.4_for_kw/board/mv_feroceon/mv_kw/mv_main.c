...
MV_BOOL isValidMacAddr(char *macAddr)
{
	int i;
...
MV_BOOL isMyDisk(int dev, char *macAddr)
{
	...
	if (!ext2fs_devread(0, 6, 17, macAddrOnDisk)) {
		printf("** Bad ext2 partition or disk - %d:%d **\n", dev, 0);
		goto _isMyDisk_end;
	}

	if (!isValidMacAddr(macAddrOnDisk)) {
		if (!ext2fs_devread(0, 0x28, 17, macAddrOnDisk)) {
			printf ("** Bad ext2 partition or disk - %d:%d **\n", dev, 0);
			goto _isMyDisk_end;
		}
	}
...
MV_STATUS serch_boot_drv(char max_disk)
{
	...
	ulong data, len, checksum, newest_disk_ihtime;
	int i ;
	...
	if (!pAddr) {
		printf("Getting MAC Address failed.\n");
		return MV_FAIL;
	}
...
ethaddr=$(ethaddr) eth1addr=$(eth1addr) ethmtu=$(ethmtu) eth1mtu=$(eth1mtu) $(netbsd_netconfig)");
	...
	setenv("bootcommon",
	       "setenv bootargs $console $bootargs_root $bootargs_func $buffalo_ver $mtdparts $tftpboot $nandboot; "
...
#endif
#if defined(CONFIG_BUFFALO_PLATFORM)
	if(mvBoardIdGet() == BF_MVAVL_ID)
	{
		env = getenv("ethaddr");
		if(!env) {
			sprintf(buff,"mv_net_config=(00:11:88:0f:62:81,0:1),mtu=1500");
		}
		else {
			sprintf(buff,"mv_net_config=(%s,0:1),mtu=1500", env);
		}
		setenv("mvNetConfig",buff);
		setenv("bootcommon",
		       "setenv bootargs $console $mvNetConfig $bootargs_root $bootargs_func $buffalo_ver $mtdparts $tftpboot $nandboot; "
		       "bootm 0x00100000 0x00800000");
	}
...
int board_late_init (void)
{
	...
	if(mvBoardIdGet() == BF_MVAVL_ID)
	{
		mvEthE6123SwitchBasicInit();
		bfGetLedBarControl(MV_TRUE);
		bfSetLedBar(100);
		for(i = 0; i < 300; i++)
			udelay(1000);
		bfSetLedBar(0);
...
static void mvHddPowerCtrl(void)
{
    ...
    {
        hddPowerBit = mvBoardGpioPinNumGet(BOARD_GPP_HDD_POWER, 0);
        fanPowerBit = mvBoardGpioPinNumGet(BOARD_GPP_FAN_POWER, 0);
        if (hddPowerBit > 31)
    	{
...
MV_BOOL bfIsStartBootProcessSlidePower(void)
{
	...
	while (!bfIsPWR_or_PWRAUTO_SignalAsserted())
		udelay(1000);
...
MV_BOOL
bfWaitUntilPushPowerButton(MV_U32 hold_usec)
{
	MV_U32 i = 0;
	MV_U32 watch_interval_usec = 1000;

	while(!bfGppInRegBitTest(BIT_PWR_SW))
		udelay(watch_interval_usec);

	for(i = 0; i < hold_usec / watch_interval_usec; i++)
	{
...
#define CNT_INTERVAL	(1 * 1000)
#define INIT_WAIT	(CNT_INTERVAL * 3 + 10 * 1000)
MV_BOOL
bfIsStartBootProcessPushPower(void)
{
	MV_U32 magic = bfGetMagicKey();
	MV_U16 i = 0;

	if(bfBuffaloFlagAtIntegRtc(GET_BUFFALO_FLAG) == MV_TRUE)
	{
		printf("doing a part of reboot process ... \n");
		bfBuffaloFlagAtIntegRtc(UNSET_BUFFALO_FLAG);
		return MV_TRUE;
	}
	printf("Doing normal bootup process ... \n");

	printf("MacicKey = 0x%02x\n", magic);
	bfSetMagicKey(MagicKeySwPoffUbootPassed);
	printf("Waiting power button would be pusshed. ... \n");

	while(!bfWaitUntilPushPowerButton(1000 * 1000 /* usec */))
	{
		// do nothinng.
	}

	bfGppOutRegBitAssert(BIT_LED_FULL_BRIGHT);
	bfSetLedBar(100);
	...
	for (i = 0; i < INIT_WAIT; i++)
	{
		if (!bfGppInRegBitTest(BIT_PWR_SW))
			break;
		if (i >= CNT_INTERVAL * 3 && (i % (CNT_INTERVAL) == 0))
			bfSetLedBar(100 - (((i - CNT_INTERVAL * 3) / (CNT_INTERVAL)) * 10));
		udelay(1000);
	}
...
MV_BOOL bfIsStartBootProcess(void)
{
	switch(mvBoardIdGet()) {
	case BF_MVAVL_ID:
		return bfIsStartBootProcessPushPower();
		;;
	case BF_MVLSXL_GE_ID:
		return MV_TRUE;
		;;
	default:
		return bfIsStartBootProcessSlidePower();
		;;
	}
}
