 ...
 * @license
 * @version $Id: application.js.php,v 1.14 2007/12/03 07:55:51 wiley Exp $
...
function ext_init(){
    ...
    datastore = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
            ...
            directory: '/',
            method: 'GET',
            params:{start:0, limit:50, dir: this.directory, requestType: 'xmlhttprequest', option:'com_extplorer', action:'getdircontents' }
        }),
    directory: '/',
    sendWhat: 'files',
    ...
    datastore.on('beforeload', function(ds, options) {
                    ...
                    options.params.requestType = 'xmlhttprequest';
    ...
    var cm = new Ext.grid.ColumnModel([{
        ...
        { dataIndex: 'is_sharename', hidden: true }
        ]);
  ...
  var gsm = ext_itemgrid.getSelectionModel();
    ...
    function handleRowClick(sm, rowIndex) {
      ...
      if( selections.length > 1 ) {
        ...
        tb.items.get('tb_new')[selections[0].get('is_writable') ? 'enable' : 'disable']();
        ...
        tb.items.get('tb_upload')[selections[0].get('is_writable')&&<?php echo ini_get('file_uploads') ? 'true' : 'false' ?> ? 'enable' : 'disable']();
        <?php if( ($GLOBALS["zip"] || $GLOBALS["tar"] || $GLOBALS["tgz"]) ) { ?>
        tb.items.get('tb_extract')[selections[0].get('is_archive') ? 'enable' : 'disable']();
        tb.items.get('tb_archive').enable();
        <?php } ?>
        tb.items.get('tb_view')[selections[0].get('is_file') ? 'enable' : 'disable']();
      } else {
        tb.items.get('tb_new').disable();
        tb.items.get('tb_edit').disable();
    ...
    var tb = new Ext.Toolbar(gridHead, [
      ...
      {// ADMIN
        ...
        tooltip: '<?php echo ext_Lang::msg('configuration', true ) ?>',
        cls:'x-btn-icon',
        handler: function() { document.location.href='<?php echo $GLOBALS['http_host']."/admin/index.php?lang=".$GLOBALS["language"] ?>'; }
      },
      '-',
      <?php
      } // end of if($admin)
  ...
  // initialize the statusbar
    ...
    function loadDir() {
      datastore.load({params:{start:0, limit:50, dir: datastore.directory, requestType: 'xmlhttprequest', option:'com_extplorer', action:'getdircontents', sendWhat: datastore.sendWhat }});
    }
    ...
    dirTree = new Ext.tree.TreePanel('dirtree', {
      ...
      loader: new Ext.tree.TreeLoader({
          ...
          requestMethod: 'GET',
          baseParams: {requestType: 'xmlhttprequest', option:'com_extplorer', action:'getdircontents', dir: '',sendWhat: 'dirs'} // custom http params
      }),
    ...
    dirTree.on('textchange', function(node, text, oldText) {
      ...
      if(node.id == 'ext_root' || node.attributes.is_sharename) {
  ...
  function dirContext(node, e ) {
    if( node.id == 'ext_root' ) return;
    // Select the node that was right clicked
    node.select();
    // Unselect all files in the grid
    ext_itemgrid.getSelectionModel().clearSelections();

    dirCtxMenu.items.get('new')[node.attributes.is_writable ? 'enable' : 'disable']();
    dirCtxMenu.items.get('rename')[!node.attributes.is_sharename&&node.attributes.is_deletable ? 'enable' : 'disable']();
    dirCtxMenu.items.get('remove')[!node.attributes.is_sharename&&node.attributes.is_deletable ? 'enable' : 'disable']();
    <?php if( ($GLOBALS["zip"] || $GLOBALS["tar"] || $GLOBALS["tgz"]) ) { ?>
    dirCtxMenu.items.get('archive')[node.attributes.is_readable ? 'enable' : 'disable']();
    <?php } ?>

    if (node.attributes.is_sharename) {
      // hide items at menu when node is share name
      dirCtxMenu.items.get('rename').hide();
      dirCtxMenu.items.get('remove').hide();
    } else {
      dirCtxMenu.items.get('rename').show();
      dirCtxMenu.items.get('remove').show();
    }
    ...
    function copymove( action ) {
      ...
      if( s ) {
        ...
        if(action=="copy") {
          statusBarMessage( '<?php echo ext_Lang::msg( 'copy_processing', true ) ?>', true );
        } else {
          statusBarMessage( '<?php echo ext_Lang::msg( 'move_processing', true ) ?>', true );
        }
        handleCallback(requestParams);
      } else {
    ...
    var dirCtxMenu = new Ext.menu.Menu({
      ...
      },
      {
        ...
        handler: function() { dirCtxMenu.hide();var num = 1; Ext.Msg.confirm('Confirm', String.format("<?php echo ext_Lang::err( 'miscdelitems' ) ?>", num ), function(btn) { deleteDir( btn, dirCtxMenu.node ) }); }
      },'-',
      <?php if( ($GLOBALS["zip"] || $GLOBALS["tar"] || $GLOBALS["tgz"]) ) { ?>
      {
        id: 'archive',
    ...
    function copymoveCtx(e){
      var target_id = e.target.id.replace( /_RRR_/g, '/' );
      var allow_mv_action = true;
      var has_copy_self = false;
      var s = e.data.selections;
      for(var i = 0, len = s.length; i < len; i++){
        if (e.data.selections[i].get('is_sharename')) {
          allow_mv_action = false;
        }
        if ("/"+e.data.selections[i].get('name') == target_id) {
          has_copy_self = true;
        }
      }

      // doesn't display CtxMenu if source which is sharename is same with target
      // which is also sharename
      if(!allow_mv_action && has_copy_self) return;
      else {
        copymoveCtxMenu.items.get('copy')[e.target.id!='ext_root'&&e.target.attributes.is_writable ? 'enable' : 'disable']();
        copymoveCtxMenu.items.get('move')[e.target.id!='ext_root'&&allow_mv_action&&e.target.attributes.is_writable ? 'enable' : 'disable']();
        if(!e.target.id!='ext_root'&&e.target.attributes.is_writable) {
    ...
    var treeroot = new Ext.tree.AsyncTreeNode({
        text: '<?php echo ext_Lang::msg('explorer', true ) ?>',
//      draggable:false,
        allowDrag:false,
        allowDrop:false,
        id:'ext_root'
    ...
    */
    chDir = function( directory ) {
      ...
      if( directory == '' || conn && !conn.isLoading()) {
        datastore.load({params:{start:0, limit:50, dir: directory, requestType: 'xmlhttprequest', option:'com_extplorer', action:'getdircontents', sendWhat: datastore.sendWhat }});
      }
     new Ext.data.Connection().request({
      url: '<?php echo basename( $GLOBALS['script_name']) ?>',
      params: { action:'chdir_event', dir: directory, requestType: 'xmlhttprequest', option: 'com_extplorer' },
      callback: function(options, success, response ) {
        if( success ) {
          ...
          document.title = '<?php echo ext_Lang::msg('explorer', true ) ?> - ' + datastore.directory;
  ...
  function expandNode( node, dir ) {
    ...
    function handleNodeClick( sm, node ) {
      if( node && node.id ) {
        ...
        tb.items.get('tb_new')[node.attributes.is_writable ? 'enable' : 'disable']();
        tb.items.get('tb_edit').disable();
        tb.items.get('tb_delete')[!node.attributes.is_sharename&&node.attributes.is_writable ? 'enable' : 'disable']();
        tb.items.get('tb_rename')[!node.attributes.is_sharename&&node.attributes.is_writable ? 'enable' : 'disable']();
        tb.items.get('tb_view').disable();
        tb.items.get('tb_download').disable();
        tb.items.get('tb_upload')[node.attributes.is_writable&&<?php echo ini_get('file_uploads') ? 'true' : 'false' ?> ? 'enable' : 'disable']();
        <?php if( ($GLOBALS["zip"] || $GLOBALS["tar"] || $GLOBALS["tgz"]) ) { ?>
        tb.items.get('tb_extract').disable();
