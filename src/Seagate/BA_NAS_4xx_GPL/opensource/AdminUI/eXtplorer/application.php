...
class ext_Lang {
  /**
   * Returns a string from $GLOBALS["__LANGUAGE"]['messages']
   *
   * @param string $msg
  ...
  /**
   * Returns a string from $GLOBALS["__LANGUAGE"]['error_msg']
   *
   * @param string $err
   * @param boolean $make_javascript_safe
   * @return string
  ...
  function _get( $array_index, $message ) {
    if( is_array( $message )) {
      return @$GLOBALS["__LANGUAGE"][$array_index][key($message)][current($message)];
    }
    return @$GLOBALS["__LANGUAGE"][$array_index][$message];
  }
  ...
  function detect_lang() {
    ...
    foreach($GLOBALS['_LANG'] as $K => $lang) {
      if(strpos($_AL, $K)===0)
        return file_exists( _EXT_PATH.'/languages/'.$K.'.php' ) ? $K : $default;
    }
