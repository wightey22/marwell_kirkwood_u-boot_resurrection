 ...
 * @license
 * @version $Id: archive.php,v 1.3 2007/12/13 08:02:50 wiley Exp $
...
class ext_Archive extends ext_Action {
  ...
  function execAction( $dir ) {
    ...
    if(!$GLOBALS["zip"] && !$GLOBALS["tgz"]) {
      ext_Result::sendResult('archive', false, ext_Lang::err( 'miscnofunc' ));
    }
    ...
    if(extGetParam($_POST, 'confirm' ) == 'true' ) {
      ...
      pass_execute($saveToDir, 'archive', 'You are not allowed to save the archive in this directory!');
      ...
      if($name=="") {
        ext_Result::sendResult('archive', false, ext_Lang::err('miscnoname'));
      }
      ...
      if( $cnt_filelist > $startfrom+$files_per_step ) {
        ...
        'newlocation' => make_link( 'download', $saveToDir, basename($archive_name) )
        );
...
<div style="width:auto;">
    ...
    <div class="x-box-ml"><div class="x-box-mr"><div class="x-box-mc">
        ...
        <h3 style="margin-bottom:5px;"><?php echo ext_Lang::msg('actarchive') ?></h3>
  ...
  new Ext.form.TextField({
    ...
    <?php } elseif($GLOBALS["acl_fullaccess"][0]) { ?>
    value: '<?php echo str_replace("'", "\'", "/".$GLOBALS["acl_fullaccess"][0] ) ?>',
    <?php } else { ?>
    value: '',
    <?php } ?>
    width: 200
