...
/**
 * @version $Id: delete.php,v 1.3 2007/09/12 06:52:26 wiley Exp $
 * @package eXtplorer
...
class ext_Delete extends ext_Action {
  ...
  function execAction($dir) {
    ...
    if(($GLOBALS["permissions"]&01)!=01)
      ext_Result::sendResult('delete', false, ext_Lang::err( 'accessfunc' ));
    ...
    for($i=0;$i<$cnt;++$i) {
      ...
      pass_execute($dir, 'delete');

      // Delete
      $ok=$GLOBALS['ext_File']->remove( $abs );

      if($ok===false || PEAR::isError( $ok )) {
        $error[$i]=ext_Lang::err( 'delitem' );
        if( PEAR::isError( $ok ) ) {
    ...
    ext_Result::sendResult('delete', true, ext_Lang::msg('success_delete_file') );
  }
}
//------------------------------------------------------------------------------
?>
