...
/**
 * @version $Id: functions.js.php,v 1.4 2007/09/11 07:32:17 wiley Exp $
 * @package eXtplorer
...
function openActionDialog( caller, action ) {
  ...
  switch( action ) {
    ...
    case 'delete':
      ...
      break;
    case 'extract':
      Ext.Msg.confirm('<?php echo ext_Lang::msg('extractlink', true ) ?>?', "<?php echo bb2html(ext_Lang::msg('extract_warning', true )) ?>", extractArchive);
      break;
    case 'download':
...
function dirselects_reload(directory) {
  var conn_dirselects = new Ext.data.Connection();

  conn_dirselects.request({
    ...
    callback: function(options, success, response ) {
      if( success ) {
        ...
        if( json.success ) {
          ...
          if( requestParams.dir ) {
            chDir( requestParams.dir );
            dirselects_reload(requestParams.dir);
          }
          dirTree.getRootNode().reload();
        } else {
          Ext.Msg.alert( 'Failure', json.error );
          statusBarMessage( json.error, false, false );
        }
      }
      else {
        Ext.Msg.alert( 'Error', 'Failed to connect to the server.');
        statusBarMessage( 'Failed to connect to the server.', false, false );
      }
...
function getRequestParams() {
  var selitems, dir, node, requestType;
  var selectedRows = ext_itemgrid.getSelectionModel().getSelections();
  if( selectedRows.length < 1 ) {
...
function deleteDir( btn, node ) {
  ...
  requestParams = getRequestParams();
  requestParams.requestType = 'xmlhttprequest';
