...
/**
 * @version $Id: functions.php,v 1.9 2007/12/19 06:55:25 wiley Exp $
 * @package eXtplorer
 ...
 */
function vardump($variable, $nest=0) {
  $space="";
  for($i=0;$i<$nest;$i++) $space.="  ";
  $nest++;
  ...
  if ($variable === true) {
    ...
    foreach ($variable as $key => $value) {
      ...
      $value = vardump($value, $nest);
      $result .= "{$space}[{$key}]{$newline}$value\n";
    }
    return $result;
  } else {
...
//------------------------------------------------------------------------------
function __log($name, $content, $reset=false){
  $type = "string";
  if ( is_array($content) ) {
    $type = "array";
    $content = "\n".vardump($content);
  ...
  $str = "[".date("H:i:s")."]{{$type}}{$name}: {$content}";
  if ($reset)
    @system("/bin/echo \"$str\">/tmp/log.explorer");
  else
    @system("/bin/echo \"$str\">>/tmp/log.explorer");
}
//------------------------------------------------------------------------------
...
function get_is_file( $abs_item) {    // can this file be edited?
  // use 'ls' command instead of is_file() to check this is file or not
  // because of it donesn't work on large file (more than 2GB)
  return (('d'==substr(@exec("/bin/ls -dl \"$abs_item\""),0,1))?(false):(true));
}
//------------------------------------------------------------------------------
...
function get_languages() {
  ...
  foreach( $langfiles as $lang ) {
    if (substr($lang, -4) != '.xml') continue;
    $lang_filename = basename( $lang, '.xml' );
    if (isset($GLOBALS['_LANG'][$lang_filename]) && $GLOBALS['_LANG'][$lang_filename] != "") {
      $langs[$lang_filename] = ucwords(str_replace( '_', ' ', $GLOBALS['_LANG'][$lang_filename]));
    }
  }
  return $langs;
}
function load_lang_rsc($lang){
  if ($lang == '') return false;

  $lang_file = _EXT_PATH."/languages/{$lang}.xml";
  if (file_exists($lang_file)) {
    $data = XML_unserialize(file_get_contents($lang_file));
    return $data['wixnas'];
  }
  return false;
}
...
function parse_file_date($date) {    // parsed file date
  return @date($GLOBALS["__LANGUAGE"]["date_fmt"],$date);
}
//------------------------------------------------------------------------------
...
function get_mime_type( $abs_item, $query) {  // get file's mimetype
  ...
  $extra = $GLOBALS['ext_File']->is_link( $abs_item ) ? ' ('.ext_Lang::mime('symlink').')' : '';
  // mime_type
...
function get_dir_selects( $dir ) {
  ...
  //get subdir at root directory first
  $subdirs = get_dir_list();

  // check permission
  $reserve_list = array();
  foreach($subdirs as $rel_path => $leaf){
    if(in_allowed($leaf)) {
      $reserve_list[$rel_path] = $leaf;
    }
  }
  $subdirs = $reserve_list;
...
function remove($item) {      // remove file / dir
  ...
  elseif( @is_dir($item)) {
    ...
    if(($handle= opendir($item))===false)
      ext_Result::sendResult('delete', false, basename($item).": ".ext_Lang::err( 'opendir' ));
    ...
    while(($file=readdir($handle))!==false) {
      ...
      if(!file_exists($new_item))
        ext_Result::sendResult('delete', false, basename($item).": ".ext_Lang::err( 'readdir' ));
...
function chmod_recursive($item, $mode) {      // chmod file / dir
  ...
  if(@is_link($item) || @is_file($item)) {
    $ok=@chmod( $item, $mode );
    if($ok) ext_Result::add_message(ext_Lang::msg('permchange').' '.$new_item);
    else ext_Result::add_error(ext_Lang::err( 'permchange' ).' '.$new_item);
  }
  elseif(@is_dir($item)) {
    if(($handle=@opendir($item))===false) {

      ext_Result::add_error(basename($item).": ".ext_Lang::err( 'opendir' ));
      return false;
    }
    ...
    while(($file=readdir($handle))!==false) {
      ...
      $new_item = $item."/".$file;
      if(!@file_exists($new_item)) {
        ext_Result::add_error(basename($item).": ".ext_Lang::err( 'readdir' ));
        continue;
      }
      //if(!get_show_item($item, $new_item)) continue;
    ...
    $ok=@chmod( $item, $mode );
    if($ok) ext_Result::add_message(ext_Lang::msg('permchange').' '.$item);
    else ext_Result::add_error(ext_Lang::err( 'permchange' ).' '.$item);
  }
...
function ext_alertBox( $msg ) {
  return ext_scriptTag('', 'Ext.Msg.alert( \''.ext_Lang::err( 'message' ).'\', \''. @mysql_escape_string( $msg ) .'\' );' );
}
