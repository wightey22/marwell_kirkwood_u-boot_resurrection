...
/**
 * @version $Id: init.php,v 1.8 2007/11/06 06:07:46 wiley Exp $
 * @package eXtplorer
...
elseif(isset($HTTP_SERVER_VARS)) {
  ...
  "You need at least PHP 4.0.0 to run this program; preferably PHP 4.4.4 or higher.");
}
//------------------------------------------------------------------------------
...
if ( substr($GLOBALS['__SERVER']["PHP_SELF"],-1)=='/' ) {
  $GLOBALS['__SERVER']["PHP_SELF"] .= 'index.php';
}

// the filename of the cuurent script: (you rarely need to change this)
// =========================================================
...
if($_SERVER['SERVER_PORT'] == $_SSL_PORT ) {
  $GLOBALS["script_name"] = "https://{$_IPADDR}".$GLOBALS['__SERVER']["PHP_SELF"];
  $GLOBALS['home_url']    = "https://{$_IPADDR}".dirname($GLOBALS['__SERVER']["PHP_SELF"]);
  $GLOBALS['http_host']   = "https://{$_IPADDR}";
} else {
  $GLOBALS["script_name"] = "http://{$_IPADDR}".$GLOBALS['__SERVER']["PHP_SELF"];
  $GLOBALS['home_url']    = "http://{$_IPADDR}".dirname($GLOBALS['__SERVER']["PHP_SELF"]);
...
require_once _EXT_PATH . '/include/bbcode.php';
require_once _EXT_PATH . '/libraries/lib_xml.php';
...
$GLOBALS["language"] = $mainframe->getUserStateFromRequest( 'language', 'lang', 'en' );
// Get Item
...
if (($GLOBALS["__LANGUAGE"] = load_lang_rsc($GLOBALS["language"]))===false) {
  if (($GLOBALS["__LANGUAGE"] = load_lang_rsc('en'))===false) { // english by default
...
if( !isset( $_REQUEST['dir'] ) ) {
  ...
  if( !empty( $dir ) && !$GLOBALS['ext_File']->file_exists( $GLOBALS['home_dir'].'/'.$dir )) {
    $dir = '';
  }
}
else {
...
if(!file_exists($GLOBALS["home_dir"])) {
  if(!file_exists($GLOBALS["home_dir"].$GLOBALS["separator"])) {
    $extra="<a href=\"".make_link("logout",NULL,NULL)."\">".ext_Lang::msg('btnlogout')."</a>";
    ext_Result::sendResult('', false, ext_Lang::err( 'home' ),$extra);
  }
}
if(!down_home($abs_dir)) {
  ext_Result::sendResult('', false, $GLOBALS["dir"]." : ".ext_Lang::err( 'abovehome' ));
}
if(!get_is_dir($abs_dir))
  if(!get_is_dir($abs_dir.$GLOBALS["separator"]))
  ext_Result::sendResult('', false, $abs_dir." : ".ext_Lang::err( 'direxist' ));
