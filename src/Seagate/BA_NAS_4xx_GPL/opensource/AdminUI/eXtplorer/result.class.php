...
/**
 * @version $Id: result.class.php,v 1.3 2007/09/10 03:57:48 wiley Exp $
 * @package eXtplorer
...
class ext_Result {
  ...
  function sendResult( $action, $success, $msg, $extra=array() ) {    // show error-message
    ...
    if( empty( $_GET['error'] )) {
    ...
    } elseif (ext_Result::count_errors()>0 || ext_Result::count_messages()>0) {
      show_header(ext_Lang::err( 'error' ));

      echo '<div class="quote">';
      echo '<a href="#errors">'.ext_Result::count_errors() .' '.ext_Lang::err( 'error' ).'</a>, ';
      echo '<a href="#messages">'.ext_Result::count_messages() .' '.ext_Lang::err( 'message' ).'</a><br />';
      echo "</div>\n";
      ...
      if( !empty( $_SESSION['ext_message'])) {
        ...
        echo "<div class=\"ext_message\"><a name=\"messages\"></a>
            <h3>".ext_Lang::err( 'message' ).":</strong>"."</h3>\n";
      ...
      if(!empty($_SESSION['ext_error'])) {
        echo "<div class=\"ext_error\"><a name=\"errors\"></a>
              <h3>".ext_Lang::err( 'error' ).":</strong>"."</h3>\n";
        ...
        echo "<a href=\"".str_replace('&dir=', '&ignore=', make_link("list", '' ))."\">".ext_Lang::err( 'back' )."</a>";
        if( !empty( $_REQUEST['extra'])) echo " - ".urldecode($_REQUEST['extra']);
        echo "</div>\n";
      }
    }
