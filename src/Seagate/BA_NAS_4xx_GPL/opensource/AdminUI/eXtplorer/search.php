...
/**
 * @version $Id: search.php,v 1.4 2007/12/19 03:52:35 wiley Exp $
 * @package eXtplorer
...
function find_item($dir,$pat,&$list,$recur) {  // find items
  ...
  if($handle===false) {
    ext_Result::sendResult('search', false, $dir.": ".ext_Lang::err( 'opendir' ));
  }
...
function get_result_table($list) {      // print table of found items
  ...
  for($i=0;$i<$cnt;++$i) {
    ...
    $abs_new_item = get_abs_item($dir,$item);

    // check permission(s)
    $rel_shrname = get_rel_shrname($abs_new_item);
    if(!in_allowed($rel_shrname)) continue; // ignore

    $img = get_mime_type($abs_new_item, "img");
    if(get_is_dir($abs_new_item)) {
      $link = make_link("list",get_rel_item($dir, $item),NULL);
    } else {
      $link = make_link('download',$dir,basename($item));
    }

    $response .= "<tr><td>" . "<img border=\"0\" ";
    $response .= "align=\"absmiddle\" src=\""._EXT_URL."/images/" . $img . "\" alt=\"\">&nbsp;";
    $response .= "<a href=\"".$link."\">";
    $response .= $s_item."</a></td><td><a href=\"" . make_link("list",(($dir[0]!='/')?('/'.$dir):$dir),null)."\">";
    $response .= (($s_dir[0]!='/')?('/'.$s_dir):$s_dir)."</a></td></tr>\n";
  }
  return $response;
}
//------------------------------------------------------------------------------
function search_items($dir) {      // search for item
  ...
  if(!empty($searchitem)) {
    $msg =ext_Lang::msg('actsearchresults');
    $msg.=": (".(($dir!="")?$dir:'/').")";
  } else {
    $msg = ext_Lang::msg('searchlink');
  }
  ...
  if($searchitem!=NULL) {
    ...
    if(count($list)>0) {
      // table header
      $response .= "<tr>\n<td width=\"42%\" class=\"header\"><b>".ext_Lang::msg('nameheader');
      $response .= "</b></td>\n<td width=\"58%\" class=\"header\"><b>".ext_Lang::msg('pathheader');
      ...
      $response .= ext_Lang::msg('miscitems').".</td><td class=\"header\"></td></tr>\n";
    } else {
      $response .= "<tr><td>".ext_Lang::msg('miscnoresult')."</td></tr>";
    }
