...
static int new_password(const struct passwd *pw, int amroot, int algo, int usestdin);
...
int passwd_main(int argc, char **argv)
{
	...
	int stdinflag = 0;			/* --stdin - read pass from stdin */
	const struct passwd *pw;

	static const struct option arg_options[] = {
		{"stdin",  no_argument, NULL, '-'},
		{0, 0, 0, 0}
	};
	...
	while ((flag = getopt_long(argc, argv, "a:dlu", arg_options, NULL)) != EOF) {
		switch (flag) {
		case 'a':
	...
	if (!(dflg || lflg || uflg)) {
		...
		if (new_password(pw, amroot, algo, stdinflag)) {
...
static int new_password(const struct passwd *pw, int amroot, int algo, int usestdin)
{
	...
	if (usestdin)
	{
		int i = read(STDIN_FILENO, pass, sizeof(pass) - 1);
		if (i < 1) {
			return 1;
		}
		pass[i - 1] = '\0';
