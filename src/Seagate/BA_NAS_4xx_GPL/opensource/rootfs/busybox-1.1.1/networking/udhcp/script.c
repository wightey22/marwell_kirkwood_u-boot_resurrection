...
static char **fill_envp(struct dhcpMessage *packet)
{
	...
	envp = xcalloc(sizeof(char *), num_options + 5 + 1);
	j = 0;
	asprintf(&envp[j++], "interface=%s", client_config.interface);
	asprintf(&envp[j++], "%s=%s", "PATH",  getenv("PATH") ? : "/bin:/usr/bin:/sbin:/usr/sbin");
  ...
  check_update_resolv_conf(&*envp, j);
...
int check_update_resolv_conf(char **envp, int count)
{
  ...
  char *saveptr;
  char envp_copy[256];
  int  env_change_flag = 0;
  char buf[80];
    
  memset(buf, 0, sizeof(buf));  
  
  for (i = 0; i < count; i++){ //search envp[]
    for (j = 0; j < sizeof(env)/sizeof(struct ENV); j++) { //search env[] and then fill-in value
      if (strstr(envp[i], env[j].name)){
        memset(envp_copy, 0, sizeof(envp_copy));
        sprintf(envp_copy, "%s", envp[i]);
        token = strtok_r(envp_copy, "=", &saveptr);
        token = strtok_r(NULL, "=", &saveptr);
        /*
                    printf("%s() - token: %s\n", __func__, token);
                    printf("%s() - before: env[%d].strvalue: %s\n", __func__, j, env[j].strvalue);
                    printf("%s() - strlen(token): %d\n", __func__, strlen(token));
                    */
        
        if (strncmp(token, env[j].strvalue, strlen(token)) != 0){
          env_change_flag = 1;
          sprintf(env[j].strvalue, "%s", token);
          //printf("%s() - after: env[%d].strvalue: %s\n", __func__, j, env[j].strvalue);
        }
      }
    }
  }
  
  envp[count] = xmalloc("update_resolv_conf=X");
  asprintf(&envp[count++], "update_resolv_conf=%d", env_change_flag);
