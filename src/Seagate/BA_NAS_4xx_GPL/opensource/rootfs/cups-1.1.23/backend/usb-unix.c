/*
 * "$Id: usb-unix.c,v 1.1.1.1.12.2 2009/02/03 08:27:04 wiley Exp $"
 *
		 ...
		 int        urisize)		/* I - Size of buffer */
{
  ...
  */
  ...
  printf("###1 mfg:%s mdl:%s attr:%s serial_number:%s port:%d \n", mfg, mdl, attr, serial_number, port);

  if ( port == 0 )
    fp = fopen("/tmp/usbprinter", "w");
  else
    fp = fopen("/tmp/usbprinter", "a+");
...
/*
 * End of "$Id: usb-unix.c,v 1.1.1.1.12.2 2009/02/03 08:27:04 wiley Exp $".
 */
