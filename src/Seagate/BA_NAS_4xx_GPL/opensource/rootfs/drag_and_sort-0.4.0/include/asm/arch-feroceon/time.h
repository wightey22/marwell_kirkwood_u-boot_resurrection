...
#define LSP_CNTMR 0
#define TICKS2USECS(x) ((x)/(t_tclk/1000000))
...
static unsigned long mv_gettimeoffset(void)
{
	unsigned int ticks;
	volatile u32 cause;
	ticks = (t_tclk / HZ) - mvCntmrRead(LSP_CNTMR);
...
static irqreturn_t
mv_timer_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
...
void __init mv_time_init(void)
{
	...
	t_tclk = mvBoardTclkGet();
	timer_reload = t_tclk / HZ;
