...
#ifdef WIX_DBG
                printf("environment.crc: 0x%x, ENV_SIZE: %d\n",environment.crc,ENV_SIZE);
                printf("environment.flags: 0x%d, environment.data: %s\n",environment.flags,environment.data);
#endif
...
#ifdef WIX_DBG
                printf("DEVOFFSET(%d): %d\n",DEVOFFSET (otherdev),otherdev);
#endif
...
#ifdef WIX_DBG
                printf("environment.crc: 0x%x\n",environment.crc);
                printf("environment.flags: 0x%x\n",environment.flags);
		int i=0;
#endif

		tmp_env = (env_t *)(environment.data_all);
		tmp_env->crc = environment.crc;
		tmp_env->flags = environment.flags;
		memcpy( environment.data_all+5, environment.data, ENV_SIZE);
#ifdef WIX_DBG
                printf("ENV_SIZE: %d\n",ENV_SIZE);
#endif
                if (write (fdr, environment.data_all, ENV_SIZE+5) != ENV_SIZE+5) {
                        fprintf (stderr,
                                "Write error on %s: %s\n",
                                DEVNAME (otherdev), strerror (errno));
                        return (-1);
...
//add for 2nd section
		...
		if (lseek (fdr, 0x4000, SEEK_SET) == -1) {
			fprintf (stderr,
				"seek error on %s: %s\n",
				DEVNAME (otherdev), strerror (errno));
			return (-1);
...
//		}
		...
		erase.start = DEVOFFSET (otherdev);
#ifdef WIX_DBG
                printf("erase.length: %d,erase.start:%d\n",erase.length,erase.start);
#endif
...
//add
	if ((addr_a= calloc (1, ENV_SIZE+5)) == NULL) {
		fprintf (stderr,
			"Not enough memory for environment (%ld bytes)\n",
			ENV_SIZE+5);
		return (errno);
