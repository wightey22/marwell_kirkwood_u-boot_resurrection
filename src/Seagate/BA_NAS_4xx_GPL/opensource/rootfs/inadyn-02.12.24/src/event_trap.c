...
static BOOL is_destination_reachable(RAS_THREAD_DATA *p_ras_thread_data,const char *test_url,int debug_level)
{
...
static BOOL is_network_up(RAS_THREAD_DATA *p_ras_thread_data,const char *test_url,int debug_level)
{
  DWORD       dwFlags=0;
  DWORD       netErrCode=0;
  DWORD       retVal=false;
...
*/
  ...
  if (!(IsNetworkAlive(&dwFlags))) {
    ...
    if (retVal) {
      ...
      retVal=is_destination_reachable(p_ras_thread_data,test_url,debug_level);
...
static BOOL is_ras_online(RAS_THREAD_DATA *p_ras_thread_data,BOOL isOptimistic,const char *test_url,int debug_level)
...
*/
  ...
  waitRet=WaitForSingleObject(hRASMutex,0);
...
BOOL is_online(RAS_THREAD_DATA *p_ras_thread_data,const char *test_url,int debug_level)
{
  ...
  if (p_osVersionInfo->dwMajorVersion<5)
    ...
    return (is_network_up(p_ras_thread_data,test_url,debug_level));

  else

    return is_ras_online(p_ras_thread_data,false,test_url,debug_level);
}
