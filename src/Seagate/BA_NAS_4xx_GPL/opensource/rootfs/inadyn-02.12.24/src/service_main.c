...
static BOOL get_reg_value(HKEY hKey,LPTSTR szSubKey,LPTSTR szValueName,char **bpRetValue)
{
  ...
  if (wcslen(szParamStr)) {
    ...
    if (*szParamStr) {
      ...
      while (1) {
        ...
        /*move to end of argument -- separated by space, or eol*/
...
static void removeServiceParams(int *argc,wchar_t *argv[],wchar_t *dest[])
{
  ...
  while (i<*argc) {
    ...
    if ((!(wcscmp(argv[i],_SERVICE_INSTALL)) || !(wcscmp(argv[i],
         _SERVICE_UNINSTALL)) || !(wcscmp(argv[i],_SERVICE_START)) || !(wcscmp(
         argv[i],_SERVICE_EXIT)) || !(wcscmp(argv[i],_SERVICE_RESTART)))) 
    {
      ...
      /*_SERVICE_INSTALL, singularly, can have a parameter*/
...
static BOOL service_command_start(int argc,wchar_t *argv[])
{
  ...
  for (i=0;i<argc;i++) free(serviceWorkaroundArgs[i]);
...
BOOL get_app_parameters(wchar_t **szParamStr,wchar_t *szServiceName,wchar_t *szRegKey,wchar_t *szRegValueName)
{
  ...
  retVal=get_reg_value(HKEY_LOCAL_MACHINE,szRegStr,szRegValueName,(LPBYTE *) szParamStr);
