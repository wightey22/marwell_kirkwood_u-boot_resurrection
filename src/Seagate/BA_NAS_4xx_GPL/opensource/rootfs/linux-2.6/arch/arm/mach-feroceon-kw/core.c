...
#ifdef SHASTA_BOARD
static void power_off_shasta_proc(void);
#endif
...
#ifdef SHASTA_BOARD
u8      shasta_hdd_uuid[4][6];
//BOARD_1A +
int     shasta_board_sample_1b=0;
//BOARD_1A -
int sata_pmode=1;
#endif
...
#ifdef SHASTA_BOARD
        for (i = 0; i < 4; i++) {
            memcpy(shasta_hdd_uuid[i], tag->u.mv_uboot.hdd_uuid[i], 6);
        }
#endif
	return 0;
}
...
static struct resource kw_i2c_resources[] = {
       {
               .name   = "i2c base",
//nick               .start  = INTER_REGS_BASE + TWSI_SLAVE_ADDR_REG,
//nick               .end    = INTER_REGS_BASE + TWSI_SLAVE_ADDR_REG + 0x20 -1,
               .start  = INTER_REGS_BASE + 0x11000,
               .end    = INTER_REGS_BASE + 0x11000 + 0x20 -1,
...
static void __init mv_init(void)
{
	...
	bootVal = MV_REG_READ(mvCtrlMppRegGet(1));
        bootVal &= 0x0f000000;
//extern int shasta_board_sample_1b;
                  if ( bootVal == 0 ) shasta_board_sample_1b = 1;
...
#endif
#ifdef SHASTA_BOARD
	pm_power_off=power_off_shasta_proc;
#endif
    return;
}
...
#define SHASTA_SHUTDOWN_PIN	48
static void power_off_shasta_proc(void)
{
	mvGppValueSet(GPP_GROUP(SHASTA_SHUTDOWN_PIN), \
	GPP_BIT(SHASTA_SHUTDOWN_PIN), 0 << GPP_ID(SHASTA_SHUTDOWN_PIN));
