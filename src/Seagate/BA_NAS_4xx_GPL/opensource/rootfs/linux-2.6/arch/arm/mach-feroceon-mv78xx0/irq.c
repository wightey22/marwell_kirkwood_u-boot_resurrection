...
{	
	if(irq < 32)
		MV_REG_BIT_RESET(CPU_INT_MASK_LOW_REG(0), (1 << irq) );
	else if(irq < 64)/* irq > 32 && irq < 64 */
		MV_REG_BIT_RESET(CPU_INT_MASK_HIGH_REG(0), (1 << (irq - 32)));
	else /* gpp */
...
static void mv_unmask_irq(unsigned int irq)
{
	...
	if (irq < 32)
		MV_REG_BIT_SET(CPU_INT_MASK_LOW_REG(0), (1 << irq));
	else if (irq < 64) /* irq > 32 && irq < 64 */
        	MV_REG_BIT_SET(CPU_INT_MASK_HIGH_REG(0), (1 << (irq - 32)));
	else
...
void __init mv_init_irq(void)
{
	...
	MV_REG_WRITE(CPU_INT_MASK_HIGH_REG(0), IRQ_GPP_MASK);	
