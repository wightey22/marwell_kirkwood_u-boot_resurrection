...
#define SENSOR_REFRESH_INTERVAL	(1 * HZ)
...
struct adt7473_data {
	...
	u8			temp_range[ADT7473_TEMP_COUNT];
...
//nick static int adt7473_probe(struct i2c_client *client,
//nick			 const struct i2c_device_id *id);
static int adt7473_attach_adapter(struct i2c_adapter *adapter);
static int adt7473_detect(struct i2c_adapter *adapter, int address, 
			int kind);
static int adt7473_detach_client(struct i2c_client *client);

static struct i2c_driver adt7473_driver = {
//nick	.class		= I2C_CLASS_HWMON,
	.driver = {
		.name	= "adt7473",
	},
//nick	.probe		= adt7473_probe,
//nick	.remove		= adt7473_remove,
//nick	.id_table	= adt7473_id,
//nick	.detect		= adt7473_detect,
//nick	.address_data	= &addr_data,
...
static void adt7473_init_client(struct i2c_client *client)
{
	...
	if (!(reg & ADT7473_CFG1_READY)) {
		...
		/* setup default pwm1 behavior */

//remove by thermal recommand		/* set max pwm duty cycle to 238 , max fan<=2000 */
//		i2c_smbus_write_byte_data(client, ADT7473_REG_PWM_MAX(0),
//				  238);

		/* set temp offset to 1 */
//		i2c_smbus_write_byte_data(client, 0x72,2);

//DVT I
//		/* set min pwm duty cycle to 130 */
//		i2c_smbus_write_byte_data(client, ADT7473_REG_PWM_MIN(0),
//				  130);
//DVT I
//DVT II +
		/* set min pwm duty cycle to 140 */
		i2c_smbus_write_byte_data(client, ADT7473_REG_PWM_MIN(0),
				  140);
		/* set max pwm duty cycle to 204(80%) */
		i2c_smbus_write_byte_data(client, ADT7473_REG_PWM_MAX(0),
				  204);
		/* set therm  to 59C */
		i2c_smbus_write_byte_data(client, ADT7473_REG_TEMP_TMAX(1),
				  59+64);
		/* set therm  to 59C */
		i2c_smbus_write_byte_data(client, ADT7473_REG_TEMP_TMAX(2),
				  59+64);

//DVT II -
		/* set min of local temperature to 47C */
		i2c_smbus_write_byte_data(client, ADT7473_REG_TEMP_TMIN(1),
				  47+64);
		/* set min of Remote2 temperature to 47C */
		i2c_smbus_write_byte_data(client, ADT7473_REG_TEMP_TMIN(2),
				  47+64);
		/* set Trange to 32 */
        	reg = i2c_smbus_read_byte_data(client,
                                       ADT7473_REG_TEMP_RANGE(2));
//DVT I                reg = ( 12 << ADT7473_TEMP_RANGE_SHIFT) |
                reg = ( 12 << ADT7473_TEMP_RANGE_SHIFT) |
              	        (reg & ~ADT7473_TEMP_RANGE_MASK);
		i2c_smbus_write_byte_data(client,
			ADT7473_REG_TEMP_RANGE(2), reg);
		...
		/* set to automatic fan control */
        	...
        	reg = (2 << ADT7473_PWM_BHVR_SHIFT) |
              		(reg & ~ADT7473_PWM_BHVR_MASK);
        	i2c_smbus_write_byte_data(client, ADT7473_REG_PWM_BHVR(0),
                                  reg);
		//dummy read
		reg = adt7473_read_word_data(client,
						ADT7473_REG_FAN(0));
	}
}
...
//		goto no_sensor_update;
	...
	for (i = 0; i < ADT7473_TEMP_COUNT; i++) {
		/*sense the data, if it is 0xff, retry 5 times */
		data->temp[i] = i2c_smbus_read_byte_data(client,
							 ADT7473_REG_TEMP(i));
		if( data->temp[i] == 0xff ) {
		    int j=0;
		    for(j=0; j<5; j++) {
			data->temp[i] =i2c_smbus_read_byte_data(client, ADT7473_REG_TEMP(i));
			if (data->temp[i] == 0xff ) continue;
		    }
		}
	}
...
no_sensor_update:
	...
	for (i = 0; i < ADT7473_TEMP_COUNT; i++) {
		...
		data->temp_range[i] = i2c_smbus_read_byte_data(client,
						ADT7473_REG_TEMP_RANGE(i));
	}
...
static ssize_t show_temp_range(struct device *dev,
			     struct device_attribute *devattr,
			     char *buf)
{
	struct sensor_device_attribute *attr = to_sensor_dev_attr(devattr);
	...
	return sprintf (buf, "%u\n", \
        (data->temp_range[attr->index] >> ADT7473_TEMP_RANGE_SHIFT));
}

static ssize_t set_temp_range(struct device *dev,
			    struct device_attribute *devattr,
			    const char *buf,
			    size_t count)
{
        ...
        mutex_lock(&data->lock);
        reg = i2c_smbus_read_byte_data(client,
                                       ADT7473_REG_TEMP_RANGE(attr->index));
        reg = (temp << ADT7473_TEMP_RANGE_SHIFT) |
              (reg & ~ADT7473_TEMP_RANGE_MASK);
        i2c_smbus_write_byte_data(client, ADT7473_REG_TEMP_RANGE(attr->index),
                                  reg);
        data->temp_range[attr->index] = reg;
        mutex_unlock(&data->lock);
			...
			char *buf)
{
	...
	if (!FAN_DATA_VALID(data->fan[attr->index])) {
		data = adt7473_update_device(dev);
	}
...
static SENSOR_DEVICE_ATTR(temp1_range, S_IWUSR | S_IRUGO, show_temp_range,
			  set_temp_range, 0);
static SENSOR_DEVICE_ATTR(temp2_range, S_IWUSR | S_IRUGO, show_temp_range,
			  set_temp_range, 1);
static SENSOR_DEVICE_ATTR(temp3_range, S_IWUSR | S_IRUGO, show_temp_range,
			  set_temp_range, 2);
...
static struct attribute *adt7473_attr[] =
{
	...
	&sensor_dev_attr_temp1_range.dev_attr.attr,
	&sensor_dev_attr_temp2_range.dev_attr.attr,
	&sensor_dev_attr_temp3_range.dev_attr.attr,
...
static int adt7473_detect(struct i2c_adapter *adapter, int address, int kind)
{
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA))
        ...
        new_client->driver = &adt7473_driver;
        new_client->flags = 0;
	...
	if (kind <= 0) {
		...
		vendor = i2c_smbus_read_byte_data(new_client, ADT7473_REG_VENDOR);
		if (vendor != ADT7473_VENDOR)
			return -ENODEV;

		device = i2c_smbus_read_byte_data(new_client, ADT7473_REG_DEVICE);
	...
	strlcpy(new_client->name, "adt7473", I2C_NAME_SIZE);
       ...
       /* Register sysfs hooks */
        ...
        err = sysfs_create_group(&new_client->dev.kobj, &data->attrs);
        if (err)
                goto exit_detach;
	...
	/* Initialize the ADT7473 chip */
        adt7473_init_client(new_client);

	data->sensors_valid = 0;	//set default as invalid

        dev_info(&new_client->dev, "%s chip found\n", new_client->name);

	return 0;

exit_remove:
        sysfs_remove_group(&new_client->dev.kobj, &data->attrs);
exit_detach:
...
/* This function is called when:
     * adt7473_driver is inserted (when this module is loaded), for each
       available adapter
     * when a new adapter is inserted (and adt7473_driver is still present) */
