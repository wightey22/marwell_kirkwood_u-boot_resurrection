 ...
 */
#ifdef SHASTA_BOARD
#define SHASTA_MAX_DISK 4
extern u8 shasta_hdd_uuid[4][6];
extern u8 mvMacAddr[CONFIG_MV_ETH_PORTS_NUM][6];
static u8 foreign_disk_bitmap = 0;
#endif
...
#ifdef SHASTA_BOARD
void prepare_uuid(void)
{
        int i;
/*      //debug usage
        printk("\nsda_uuid=");
        for(i=0;i<6;i++)
            printk("0x%2.2x ", shasta_hdd_uuid[0][i]);
        printk("\nsdb_uuid=");
        for(i=0;i<6;i++)
            printk("0x%2.2x ", shasta_hdd_uuid[1][i]);
        printk("\nsdc_uuid=");
        for(i=0;i<6;i++)
            printk("0x%2.2x ", shasta_hdd_uuid[2][i]);
        printk("\nsdd_uuid=");
        for(i=0;i<6;i++)
            printk("0x%2.2x ", shasta_hdd_uuid[3][i]);
        printk("\neth0 mac= ");
        for(i=0;i<6;i++)
            printk("0x%2.2x ", mvMacAddr[0][i]);
        printk("\n");
*/
        if (foreign_disk_bitmap & 0x80) return;
        foreign_disk_bitmap=0;
        for ( i=0; i<4; i++) {
            if (memcmp(shasta_hdd_uuid[i], mvMacAddr[0], 6)) {
                foreign_disk_bitmap |= (1<<i);
                if(shasta_hdd_uuid[i][0] == 0xff)
                    printk(KERN_INFO "Disk %d Not Found\n", i);
                else
                    printk(KERN_INFO "Found Foreign Disk %d\n", i);
            }
        }
        foreign_disk_bitmap |=0x80;
}
#endif
...
void md_autodetect_dev(dev_t dev)
{
	if (dev_cnt >= 0 && dev_cnt < 127)
        ...
        {
            prepare_uuid();
//          printk(KERN_INFO "===== Enter md_autodetect_dev ======\n");
//          printk(KERN_INFO "Foreign disk map = %x\n", foreign_disk_bitmap);
            int i, j;
            u8 tmp;
            i = MINOR(dev)/16;
            j = 0;
            // try to find out how many exactly mapping
            if(i<4) {
                for(j=0; j<4 ;j++) {
                    if( shasta_hdd_uuid[j][0] == 0xff ) {
                        continue;
                    } else {
                        if(--i < 0) break;
                    }
...
//          printk(KERN_INFO " j = %d\n", j);
                if ( foreign_disk_bitmap & (1<<j) ) return;
            }
