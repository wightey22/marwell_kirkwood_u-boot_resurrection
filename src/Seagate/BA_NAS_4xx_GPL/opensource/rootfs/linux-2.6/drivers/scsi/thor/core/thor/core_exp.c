...
/* For Thor, I change MV_ASSERT related to hardware status to MV_DASSERT. */
...
#if 1 //def SUPPORT_SCSI_PASSTHROUGH //SHASTA_BOARD
// Read TaskFile
...
void PATA_LegacyPollSenseData(PCore_Driver_Extension pCore, PMV_Request pReq)
{
	...
	//Let me fake the sense data first to see what's gonna happen.
...
void mvScsiReadCapacity_16(PCore_Driver_Extension pCore, PMV_Request pReq)
{
	...
	pU32Buffer = (MV_PU32)pReq->Data_Buffer;
	pU32Buffer[0] = CPU_TO_BIG_ENDIAN_32(maxLBA.parts.low);
	pU32Buffer[1] = CPU_TO_BIG_ENDIAN_32(maxLBA.parts.high);
	...
	)
{
	...
	if(!hotPlugDevice && !hotPlugPM){
		MV_DPRINT(("No any device hot plug on port[%d], waiting.\n",pPort->Id));
		HBA_SleepMillisecond(pCore, 1000);
		intStatus = MV_REG_READ_DWORD(portMmio, PORT_IRQ_STAT);
		hotPlugDevice = intStatus & PORT_IRQ_PHYRDY;
	...
	)
{
	...
	{
		MV_PRINT("Interrupt Error: 0x%x orgIntStatus: 0x%x completeSlot=0x%x on port[%d].\n", 
...
#ifdef SHASTA_BOARD
extern int sata_pmode;
extern unsigned long pmode_time_stamp;
#endif
...
#ifdef SHASTA_BOARD
        /* Do something if necessary to return back the request. */
        if ( (pReq->Cdb[0]==SCSI_CMD_MARVELL_SPECIFIC) && (pReq->Cdb[1]==CDB_CORE_MODULE) && (pReq->Cdb[2] == CDB_CORE_ATA_CHECK_POWER_MODE)) {
                if (pTaskFile->Sector_Count == 0)
                        sata_pmode=0;   //0=standby
                else
                        sata_pmode=1;   //1=active
                pmode_time_stamp=jiffies;
	}
#endif
	if(pReq->Completion)
