...
/* do not need to release since we are using fixed tag number 31 */
/*	Tag_ReleaseOne(&pPort->Tag_Pool, tag); */
	mvEnableIntr(portMmio, old_stat);
	
}
MV_U8 mvGetSataDeviceType_6121(PDomain_Port pPort)	//SHASTA_BOARD
{
	MV_LPVOID portMmio = pPort->Mmio_Base;
	MV_U32 tmp;
...
MV_BOOLEAN SATA_SoftResetDevice(PDomain_Port pPort, MV_U8 portNum)
{
	...
	while(loop>0)
	{
		...
		{
			MV_DPRINT(("loop = %x\n", loop));
			pPort->Type = mvGetSataDeviceType_6121( pPort ); //SHASTA_BOARD
			return MV_TRUE;
		}
...
#ifndef COMMAND_ISSUE_WORKROUND
