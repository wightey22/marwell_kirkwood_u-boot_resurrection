			      ...
			      PMV_SG_Table sg_table)
{
	...
	if (mv_use_sg(scmd)) {
		...
		if (MV_SCp(scmd)->mapped == 0) {
			busaddr = pci_map_single(phba->dev,
				      mv_rq_bf(scmd),
				       mv_rq_bf_l(scmd),
					   scsi_to_pci_dma_dir(scmd->sc_data_direction));
				...
				PMV_Request pReq)
{
	...
	if (0) {
#else
	if (scmd  &&  (SCSI_CMD_MODE_SENSE_6 == scmd->cmnd[0]) && mv_use_sg(scmd)) {
#endif
		int i;
		struct scatterlist *sg = (struct scatterlist *) mv_rq_bf(scmd);
		unsigned char *buf=(unsigned char *) map_sg_page(sg) + sg->offset;
		/* 
...
/* !DMA_NONE */
	if (MV_SCp(scmd)->mapped) {
		if (mv_use_sg(scmd))
			...
			pci_unmap_single(phba->dev,MV_SCp(scmd)->bus_address,
				...
				PMV_Request pReq)
{
	...
	case READ_TOC:
		...
		pReq->Cdb[2] = scmd->cmnd[9]>>6;
		pReq->Cdb[6] = scmd->cmnd[6];
		pReq->Cdb[7] = scmd->cmnd[7];
		pReq->Cdb[8] = scmd->cmnd[8];
		break;
...
#ifdef SHASTA_BOARD
int     sata_in_standby = 0;
int     standby_timer_running = 0;
#endif
				  ...
				  void (*done) (struct scsi_cmnd *))
{
	...
	MV_DBG(DMSG_SCSI_FREQ,
	       ...
	       cdb=(%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x))\n", 
	...
	if(mv_core_check_is_reseeting(core_desc->extension)){
        ...
        if(sata_in_standby) {
            if ( (*(scmd->cmnd) == 0x85) &&
                 ...
                 (*(scmd->cmnd+2) == 0x2c) &&
                 (*(scmd->cmnd+3) == 0x00) &&
                 (*(scmd->cmnd+14) == 0xe5)) goto skip_standby;
            if ( (*(scmd->cmnd) == 0x0c) &&
                 (*(scmd->cmnd+1) == 0x00) &&
                 (*(scmd->cmnd+2) == 0x00) &&
                 (*(scmd->cmnd+3) == 0x00) &&
            ...
            if ( (*(scmd->cmnd) == 0x85) &&
                 ...
                 (*(scmd->cmnd+2) == 0x0c) &&
                 (*(scmd->cmnd+3) == 0x00) &&
                 (*(scmd->cmnd+14) == 0xe0)) goto skip_standby;
            if ( (*(scmd->cmnd) == 0x0c) &&
                 (*(scmd->cmnd+1) == 0x00) &&
                 (*(scmd->cmnd+2) == 0x00) &&
                 (*(scmd->cmnd+3) == 0x00) &&
                ...
                standby_timer_running++;
                printk("standby_timer_running = %d\n", standby_timer_running);
                        msleep(3000);
                        if(standby_timer_running >=6) {
                                sata_in_standby=0;
                                standby_timer_running=0;
                        }
                        return SCSI_MLQUEUE_HOST_BUSY;
        }
skip_standby:
...
#ifdef RAID_DRIVER
	pAdInfo->AlarmSupport = MV_TRUE;
	/* Check if Adapter is running in Non-RAID mode */
#ifdef ODIN_DRIVER
	if (!pHBA->RunAsNonRAID) 
#endif
	{
		pRaidCore = (PRAID_Core)HBA_GetModuleExtension(This,MODULE_RAID);
		pAdInfo->MaxTotalBlocks = pRaidCore->BLK_Count_Supported;
		pAdInfo->MaxBlockPerPD = MAX_BLOCK_PER_HD_SUPPORTED;		// hardcoded to 8 for now
		pAdInfo->MaxLogicalDrive = pRaidCore->LD_Count_Supported;

		/* Set BGA features */
		/* BGA features may vary with hardware ID */
		/* Currently support all BGA features */
		pRaidCore->BGA_Capability = FEATURE_BGA_REBUILD_SUPPORT | FEATURE_BGA_BKINIT_SUPPORT |
									FEATURE_BGA_SYNC_SUPPORT;
	#ifdef SUPPORT_MIGRATION
		pRaidCore->BGA_Capability |= FEATURE_BGA_MIGRATION_SUPPORT;
	#endif
	#ifdef SUPPORT_MP
		pRaidCore->BGA_Capability |= FEATURE_BGA_MEDIAPATROL_SUPPORT;
	#endif

		pAdInfo->Features |= pRaidCore->BGA_Capability;
		/* About Logical Driver Mode */
	#ifdef SUPPORT_RAID6
		#ifdef ODIN_DRIVER
		pAdInfo->LogicalDriverMode = SUPPORT_LD_MODE_RAID0 | SUPPORT_LD_MODE_RAID1
								   | SUPPORT_LD_MODE_RAID10;
		if ( (pHBA->device!=DEVICE_ID_6340) && (pHBA->device!=DEVICE_ID_6320) )
			/* Odin support RAID5, but Odin Lite does not */
			pAdInfo->LogicalDriverMode |= SUPPORT_LD_MODE_RAID5;
		#ifdef SUPPORT_RAID1E
			pAdInfo->LogicalDriverMode |= SUPPORT_LD_MODE_RAID1E;
		#endif
		#else	/* ODIN_DRIVER */
		pAdInfo->LogicalDriverMode = SUPPORT_LD_MODE_RAID0 | SUPPORT_LD_MODE_RAID1 |
									SUPPORT_LD_MODE_RAID5 | SUPPORT_LD_MODE_RAID10;
		#endif
		
		#ifdef RAID6_MULTIPLE_PARITY
		#ifdef SOFTWARE_XOR
		/* Software XOR supports RAID 6 GF XOR. */
		pAdInfo->LogicalDriverMode |= SUPPORT_LD_MODE_RAID6;
		#else
		/* If it's hardware XOR and A1 board, RAID6 is not supported. */
