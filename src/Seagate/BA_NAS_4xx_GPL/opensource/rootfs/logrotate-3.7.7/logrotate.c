...
int main(int argc, const char **argv)
{
    ...
    while (1) {
      ...
      while (1) { // check hdd standby
          system("/usr/sbin/sendidle -p /dev/sda");
          if ( (cmdfp=fopen("/proc/sata/pmode", "r")) != NULL ) {
              fgets( cmdbuf, sizeof(cmdbuf), cmdfp );
              fclose(cmdfp);
              if ( strstr(cmdbuf,"standby") ) {
                  sleep(60);
              } else {
                  break; // hdd doesn't get into standby mode
              }
          }
      }
