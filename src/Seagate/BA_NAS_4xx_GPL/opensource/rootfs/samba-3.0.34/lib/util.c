...
char *lock_path(const char *name)
{
	...
	/*
	 * Can we mount /usr/var/locks using tmpfs?
	 * smb & nmbd put their db, etc on there.
	 * It seems all can be recreated after system restarts.
	 * At least, browse.dat & unexpected.tdb should be.
	 * Otherwise nmbd will cause hdd spin down failure.
	 */
	if(strcmp("unexpected.tdb",name) == 0
		|| strcmp(SERVER_LIST,name) == 0)
	{
		pstrcpy(fname,"/dev/shm");
	}
	else
	{
