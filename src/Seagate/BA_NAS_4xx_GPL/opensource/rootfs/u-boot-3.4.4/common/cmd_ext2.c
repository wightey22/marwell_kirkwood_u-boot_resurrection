...
int failsafe(void)
{
        ...
        for ( dev_cnt=0 ; dev_cnt < 4 ; dev_cnt++ ){
	...
	if ( dev_cnt < 2){
        ...
        if (dev_desc==NULL) {
                if (dev_cnt ==3 )
                        printf ("\n** Block device ide %d not supported\n", dev);
                if (dev_cnt ==3 )
                        return(1);
        }
        if (part != 0) {
        ...
        if (!ext2fs_mount(part_length)) {
                if (dev_cnt ==3 )
                        printf ("** Bad ext2 partition or disk - ide %d:%d **\n",  dev, part);
                ext2fs_close();
                if (dev_cnt ==3 )
                        return(1);
        }
