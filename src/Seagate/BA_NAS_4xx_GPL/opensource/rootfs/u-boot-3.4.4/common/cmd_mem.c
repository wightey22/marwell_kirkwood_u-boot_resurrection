...
#ifdef SHASTA_BOARD
int retry_count=0;
extern nand_layout_t nand_layout_info[];
int do_check_crc32 (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    
	int i;
	char **av;
	nand_layout_t *nand_layout_ptr;
	nand_info_t *nand;
	unsigned long crc, mem_addr;
	char* s;
	...
	if( !strncmp(*av, "PrEr", 4))
	    mem_addr=0x800000;
	else 	
	    mem_addr=0x40000;
        printf ("Calculate CRC32:\n");
	for (i=0 ; i < 4; i++, nand_layout_ptr++) {
            if( !strncmp (nand_layout_ptr->sign, *av, 4)) break;
	}
//	printf("actually_offset+0x4000=%x  len=%x  crc=%x\n", nand_layout_ptr->actually_offset+0x4000, 
//				nand_layout_ptr->len, nand_layout_ptr->crc32_data);
	if (nand_layout_ptr->crc32_data == 0) {
	    puts("No crc32 checksum available on header location, skip calculating it\n");
	} else {
	    crc =crc32(0, (const uchar *) mem_addr, nand_layout_ptr->len);
	    if ( crc != nand_layout_ptr->crc32_data) {
	    	puts("crc32 checksum fail\n");
		retry_count++;
		if(retry_count > 5) {
		    //blink error led
		    unsigned ttt=0x81;
...
#ifdef SHASTA_BOARD
U_BOOT_CMD(
	check_crc32,    2,    1,     do_check_crc32,
	"check_crc32   - checksum calculation\n",
	"signature\n   - compute shasta CRC32 checksum \n"
);
#endif
