...
void scsi_scan(int mode)
{
	...
	scsi_max_devs=0;
#ifdef SHASTA_BOARD
	/* add delay while HDD on 6281 are not present */
	if ( (check_ide_hdd_presented(0)==0) && (check_ide_hdd_presented(1)==0))
		udelay(8000000);
#endif
...
removable:
	...
	if(scsi_max_devs>0)
		...
		scsi_curr_dev=-1;
#ifdef SHASTA_BOARD
	if ( (scsi_dev_desc[0].type==DEV_TYPE_UNKNOWN) && 
	     (scsi_dev_desc[1].type==DEV_TYPE_UNKNOWN))
	{
		scsi_curr_dev=-1;
	}
	else if (scsi_dev_desc[0].type==DEV_TYPE_UNKNOWN)
	{
		scsi_curr_dev=1;
	}
#endif
...
void scsi_setup_write_ext(ccb * pccb, unsigned long start, unsigned short blocks)
{
        ...
        pccb->cmdlen=10;
        pccb->msgout[0]=SCSI_IDENTIFY; /* NOT USED */
        debug ("scsi_setup_write_ext: cmd: %02X %02X startblk %02X%02X%02X%02X blccnt %02X%02X\n",
                pccb->cmd[0],pccb->cmd[1],
                pccb->cmd[2],pccb->cmd[3],pccb->cmd[4],pccb->cmd[5],
                pccb->cmd[7],pccb->cmd[8]);
}

ulong scsi_write(int device, ulong blknr, ulong blkcnt, ulong *buffer)
{
        ulong start,blks, buf_addr;
        unsigned short smallblks;
        ccb* pccb=(ccb *)&tempccb;
        ...
        start=blknr;
        blks=blkcnt;
        debug ("\nscsi_write: dev %d startblk %lx, blccnt %lx buffer %lx\n",device,start,blks,(unsigned long)buffer);
        do {
                pccb->pdata=(unsigned char *)buf_addr;
                if(blks>SCSI_MAX_READ_BLK) {
                        pccb->datalen=scsi_dev_desc[device].blksz * SCSI_MAX_READ_BLK;
                ...
                debug ("scsi_write_ext: startblk %lx, blccnt %x buffer %lx\n",start,smallblks,buf_addr);
                if(scsi_exec(pccb)!=TRUE) {
                        scsi_print_error(pccb);
                        blkcnt-=blks;
                        break;
        ...
        debug ("scsi_write_ext: end startblk %lx, blccnt %x buffer %lx\n",start,smallblks,buf_addr);
        return(blkcnt);
}
#endif
...
#ifdef SHASTA_BOARD
//query disk present or not
int	check_scsi_hdd_presented(int i)
{
	    if(scsi_dev_desc[i].type == DEV_TYPE_UNKNOWN)
		return 0;
	    else 
