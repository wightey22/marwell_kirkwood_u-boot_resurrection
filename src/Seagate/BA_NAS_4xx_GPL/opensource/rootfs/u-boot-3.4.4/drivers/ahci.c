...
static 	struct ahci_probe_ent iii;
struct ahci_probe_ent *probe_ent = &iii;
#else
...
#ifdef SHASTA_BOARD
static int ata_scsiop_write10(ccb * pccb)
{
	u64 lba = 0;
	u32 len = 0;
	u8 fis[20];
	...
	if (get_ahci_device_data(pccb->target, (u8 *) & fis, 20,
				 pccb->pdata, pccb->datalen)) {
		debug("scsi_ahci: SCSI WRITE10 command failure.\n");
		return -EIO;
	}
...
#ifdef SHASTA_BOARD
	*(u32 *) buf = __swab32(ataid[pccb->target]->lba48_capacity[0] | \
				ataid[pccb->target]->lba48_capacity[1] << 16);
#else
