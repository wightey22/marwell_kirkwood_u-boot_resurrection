...
extern struct vsf_share_acl p_share_acl[VSFTP_SHARE_ACL_MAX];
extern int share_acl_number;
...
int has_share_acl_for_cur_usr(struct mystr* cur_usr, struct mystr* p_str)
{
  ...
  // get shares list depended on the specified user
  for (i = 0; i < share_acl_number; i++) {
    if (!str_isempty(&(p_share_acl[i].login_user))) {
      if (str_equal(&(p_share_acl[i].login_user), cur_usr))  {
        str_copy(p_str, &(p_share_acl[i].share_list));
        return 1;
      }        
    }
  }

  // don't find any shares list for this user
  return 0;
}

int is_ftp_root_dir(struct mystr* p_str)
{
  if (str_getlen(p_str) == 1 && str_get_char_at(p_str, 0) == '/') {
    return 1; // in root directory '/'
  }
...
{
  struct mystr root_dir_name = INIT_MYSTR;
  struct mystr left_name = INIT_MYSTR;
  struct mystr right_name = INIT_MYSTR;
  struct mystr looking_str = INIT_MYSTR;
  struct str_locate_result loc_res;

  if (str_getlen(cur_dir) > 1 && str_get_char_at(cur_dir, 0) == '/') {
    str_right(cur_dir, &root_dir_name, str_getlen(cur_dir) - 1);
    str_copy(&left_name, &root_dir_name);
    str_split_char(&left_name, &right_name, '/');
    str_copy(&root_dir_name, &left_name);

    // make looking string for searching
    str_alloc_text(&looking_str, ":");
    str_append_str(&looking_str, &root_dir_name);
    str_append_text(&looking_str, ":");

    loc_res = str_locate_str(share_acl, &looking_str);
    if (loc_res.found)
    	return 1;
  }
                         ...
                         int is_verbose)
{
  ...
  struct mystr share_list_of_cur_user = INIT_MYSTR;
  struct mystr current_working_path = INIT_MYSTR;
  ...
  int F_option;
  int do_stat = 0;
  int in_ftp_root_dir = 0;

  if (tunable_share_acl_enable) {
    str_getcwd(&current_working_path);
    str_replace_text(&current_working_path, "\"", "\"\""); // double up any double-quotes in the pathname!

    // get shares list depended on the specified user
    if (!has_share_acl_for_cur_usr(&p_sess->user_str, &share_list_of_cur_user)) {
      return;
    }

    if (!(in_ftp_root_dir = is_ftp_root_dir(&current_working_path))) {
      if (!is_available_dir(&current_working_path, &share_list_of_cur_user)) {
        return;
      }   
    }
  }
  ...
  {
    int len;
    static struct str_locate_result shr_loc_res;
    static struct mystr s_look_share_str;
    ...
    {
      if (in_ftp_root_dir)
      { // make looking share string for comparing
        str_alloc_text(&s_look_share_str, ":");
        str_append_str(&s_look_share_str, &s_next_filename_str);
        str_append_text(&s_look_share_str, ":");
        shr_loc_res = str_locate_str(&share_list_of_cur_user, &s_look_share_str);
      }
    }
