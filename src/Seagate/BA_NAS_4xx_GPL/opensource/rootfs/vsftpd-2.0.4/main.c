...
struct vsf_share_acl p_share_acl[VSFTP_SHARE_ACL_MAX];
int share_acl_number;
...
main(int argc, const char* argv[])
{
  ...
  /* Parse share ACL conf if tunable_share_acl_enable = 1 */
  if (tunable_share_acl_enable)
  {
    share_acl_number = 0;
    struct vsf_sysutil_statbuf* p_statbuf = 0;
    int retval = vsf_sysutil_stat(tunable_share_acl_file, &p_statbuf);
    if (!vsf_sysutil_retval_is_error(retval))
    {      
       vsf_parse_share_acl_conf(tunable_share_acl_file, 1, &p_share_acl);
    }
    else
    {
      die2("vsftpd: cannot open file:", tunable_share_acl_file);
    }
    vsf_sysutil_free(p_statbuf);
  }
