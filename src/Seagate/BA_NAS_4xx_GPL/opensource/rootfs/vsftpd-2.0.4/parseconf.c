...
static void handle_share_acl_setting(struct mystr* p_setting_str,
                                  struct mystr* p_value_str,
                                  struct vsf_share_acl *p_share_acl);
...
parseconf_bool_array[] =
{
  ...
  { "share_acl_enable", &tunable_share_acl_enable },
  { 0, 0 }
};
...
parseconf_str_array[] =
{
  ...
  { "share_acl_file", &tunable_share_acl_file },
  { 0, 0 }
};
...
void
vsf_parse_share_acl_conf(const char* p_filename, int errs_fatal, struct vsf_share_acl **p_share_acl)
{
  ...
  if (!p_filename)
  {
    bug("null filename in vsf_parse_share_acl_conf");
  }
  if (!s_strings_copied)
  {
    s_strings_copied = 1;
  ...
  struct vsf_share_acl *ptr = p_share_acl;
  while (str_getline(&config_file_str, &config_setting_str, &str_pos))
  {
    ...
    if (!str_isempty(&config_setting_str)) {
      handle_share_acl_setting(&config_setting_str, &config_value_str, ptr);
      share_acl_number++;
      ptr++;
    }
  }
...
static void
handle_share_acl_setting(struct mystr* p_setting_str, struct mystr* p_value_str, struct vsf_share_acl *p_share_acl)
{
  if (!str_isempty(p_setting_str)) {
	str_copy(&p_share_acl->login_user, p_setting_str);
	str_copy(&p_share_acl->share_list, p_value_str);
  }
}
...
//    (void) vsf_sysutil_write_loop(VSFTP_COMMAND_FD, str_getbuf(p_setting_str), str_getlen(p_setting_str));
//    (void) vsf_sysutil_write_loop(VSFTP_COMMAND_FD, "\r\n------------------------------\r\n", 34);


    while (p_str_setting->p_setting_name != 0)
    {
//      (void) vsf_sysutil_write_loop(VSFTP_COMMAND_FD, p_str_setting->p_setting_name, vsf_sysutil_strlen(p_str_setting->p_setting_name));
//      (void) vsf_sysutil_write_loop(VSFTP_COMMAND_FD, "\r\n", 2);
