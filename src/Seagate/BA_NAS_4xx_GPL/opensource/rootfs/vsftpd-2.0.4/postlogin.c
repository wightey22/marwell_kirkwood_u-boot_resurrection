...
static void handle_opts(struct vsf_session* p_sess);
...
//             (tunable_anon_upload_enable || !p_sess->is_anonymous) &&
             (p_sess->anon_upl_enable || !p_sess->is_anonymous) &&
...
//             (tunable_anon_mkdir_write_enable || !p_sess->is_anonymous) &&
             (p_sess->anon_md_enable || !p_sess->is_anonymous) &&
...
//             (tunable_anon_other_write_enable || !p_sess->is_anonymous) &&
             (p_sess->anon_oth_enable || !p_sess->is_anonymous) &&
...
handle_cwd(struct vsf_session* p_sess)
{
  ...
  if (tunable_share_acl_enable) {
    ...
    if (!has_share_acl_for_cur_usr(&p_sess->user_str, &share_list_of_cur_user)) {
      goto handle_cwd_failed;
    }

    if (!is_ftp_root_dir(&current_working_path)) {
...
handle_cwd_failed:
  vsf_cmdio_write(p_sess, FTP_CWDFAIL, "Failed to change directory.");
}
...
handle_pasv(struct vsf_session* p_sess, int is_epsv)
{
  ...
  int bind_retries = (tunable_bind_retries) ? tunable_bind_retries : 10;
  ...
  if (p_sess->pasv_address != 0) {
    vsf_sysutil_sockaddr_set_ipv4addr(s_p_sockaddr, (const unsigned char*)&p_sess->pasv_address);
  } else
  if (tunable_pasv_address != 0)
  {
    /* get public IP when tunable_pasv_address = UPnP */
    static struct mystr s_cfg_pasv_address;
    str_alloc_text(&s_cfg_pasv_address, tunable_pasv_address);
    str_upper(&s_cfg_pasv_address);
    if(str_equal_text(&s_cfg_pasv_address, "UPNP")) {
      const char *multicastif = 0;
      struct UPNPDev *devlist = 0;

      if((devlist = upnpDiscover(2000, multicastif))) {
        struct UPNPUrls urls;
        struct IGDdatas data;
        char lanaddr[16], externaladdr[16];

        if(UPNP_GetValidIGD(devlist, &urls, &data, lanaddr, sizeof(lanaddr))) {
          UPNP_GetExternalIPAddress(urls.controlURL, data.servicetype, externaladdr);

          if(externaladdr[0])
            tunable_pasv_address = vsf_sysutil_strdup(externaladdr);
          else
            tunable_pasv_address = 0; // Failed to get public IP
        }
      }
    }
...
//  if (is_unique || (p_sess->is_anonymous && !tunable_anon_other_write_enable))
  if (is_unique || (p_sess->is_anonymous && !p_sess->anon_oth_enable))
  {
...
handle_mkd(struct vsf_session* p_sess)
{
  ...
  if (retval != 0)
  {
    vsf_cmdio_write(p_sess, FTP_MKDFAIL,
...
handle_rmd(struct vsf_session* p_sess)
{
  ...
  if (retval != 0)
  {
    vsf_cmdio_write(p_sess, FTP_RMDFAIL,
...
handle_opts(struct vsf_session* p_sess)
{
  ...
  {
    vsf_cmdio_write(p_sess, FTP_BADOPTS, "Option not understood_.");
  }
...
handle_stat(struct vsf_session* p_sess)
{
  ...
  if (tunable_convert_charset_enable)
  {
    ...
    vsf_cmdio_write_raw(p_sess, vsf_charconv_charset_name(tunable_local_codepage));
    vsf_cmdio_write_raw(p_sess, "\r\n");
    vsf_cmdio_write_raw(p_sess, "     Remote charset is ");
    vsf_cmdio_write_raw(p_sess, vsf_charconv_charset_name(p_sess->remote_charset));
    vsf_cmdio_write_raw(p_sess, "\r\n");
